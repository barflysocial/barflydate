...
const APP_VERSION = "BARFLYDATE v102";
...
function BusinessDemo() {
  ...
  const contactRows = [
    { label: "Phone", value: settings?.businessPhone || BUSINESS_PHONE },
    { label: "Email", value: settings?.businessEmail || BUSINESS_EMAIL },
    { label: "Website", value: settings?.website || settings?.websiteUrl || BUSINESS_WEBSITE_URL },
    { label: "Games", value: settings?.gamesSite || settings?.gamesUrl || BUSINESS_GAMES_URL },
    { label: "Instagram", value: settings?.instagramHandle || settings?.instagramUrl || BUSINESS_INSTAGRAM_URL }
  ...
  return <div className="screen publicScreen neonPublic businessDemo simplifiedDemoPage">
    ...
    <div id="business-contact" className="card contactCard demoBottomActions simplifiedContactCard">
      ...
      <div className="contactInfoGrid">
        {contactRows.map(row => <div key={row.label}>
          <b>{row.label}</b>
          <span>{row.value}</span>
        </div>)}
      </div>
      ...
      <div className="demoQrContactBox">
        <div>
          <div className="brandMark">QR CONTACT</div>
          <h2>Scan to open this demo page</h2>
          <p className="muted">{`${window.location.origin}/demo`}</p>
        </div>
        <img src={qrApiUrl(`${window.location.origin}/demo`)} alt="Demo page QR code" />
      </div>

      <div className="ctaRow demoEnterAppRow">
