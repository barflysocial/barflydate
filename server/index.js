...
function normalizeSettings(body = {}) {
  ...
  return {
    ...
    homepageDestination: String(body.homepageDestination ?? current.homepageDestination ?? "events").trim(),
...
function normalizeBookingRequest(body = {}, existing = {}) {
  const eventScope = ["private", "public"].includes(body.eventScope) ? body.eventScope : (existing.eventScope || "public");
  return {
...
function publicEvent(event) {
  ...
  return {
    ...event,
    eventTypeLabel: eventTypeLabels[event.eventType] || "Event",
...
app.get("/api/host/bookings/export", requireHost, (req, res) => {
  const rows = [
    ["Name","Type","Date","Time","Duration","Phone","Email","Status","Notes","Created At"],
...
app.post("/api/host/events", requireHost, (req, res) => {
  ...
  res.json({ success: true, event: publicEvent(event), events: publicEvents({ includeHidden: true }) });
});
...
app.post("/api/host/events/:eventId", requireHost, (req, res) => {
  ...
  res.json({ success: true, event: publicEvent(updated), events: publicEvents({ includeHidden: true }) });
});
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v40 running on port ${PORT}`));
}).catch(err => {
