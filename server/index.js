...
function rsvpsForGame(gameId) {
  return (db.rsvps || []).filter(rsvp => rsvp.gameId === gameId && rsvp.status !== "cancelled");
}
...
app.post("/api/rsvps/:rsvpId/change-session", (req, res) => {
  ...
  if (isGameStarted(originalGame)) {
    return res.status(400).json({ error: "Your original session has already started. Your RSVP is locked for this event." });
  }
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v19 running on port ${PORT}`));
}).catch(err => {
