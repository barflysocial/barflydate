...
app.post("/api/demo-game", requireHost, (req, res) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const player = {
      ...
      realName: `${demoFirstNames[i % demoFirstNames.length]} Demo`,
      contactHandle: `demo${i + 1}@barflydate.test`,
      gender: demoGenders[i % demoGenders.length],
      interestedIn: demoInterested[(i + 1) % demoInterested.length],
      lookingFor: demoLooking[i % demoLooking.length],
      interests,
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v13 running on port ${PORT}`));
}).catch(err => {
