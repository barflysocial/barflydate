...
const COHOST_PIN = process.env.COHOST_PIN || "1357";
...
function isCohost(req) {
  const pin = String(req.headers["x-cohost-pin"] || req.query.cohostPin || "");
  return pin === COHOST_PIN || isHost(req);
}
...
function requireCohost(req, res, next) {
  if (!isCohost(req)) {
    return res.status(401).json({ error: "Co-host PIN required" });
  }
  next();
}
...
function normalizeSettings(body = {}) {
  ...
  return {
    ...
    homepageDestination: String(body.homepageDestination ?? current.homepageDestination ?? "home").trim(),
    ...
    appIconPopupActive: Boolean(body.appIconPopupActive ?? current.appIconPopupActive ?? false),
    appIconPopupTitle: String(body.appIconPopupTitle ?? current.appIconPopupTitle ?? "").trim(),
    appIconPopupText: String(body.appIconPopupText ?? current.appIconPopupText ?? "").trim(),
    appIconPopupImageUrl: String(body.appIconPopupImageUrl ?? current.appIconPopupImageUrl ?? "").trim(),
    appIconPopupButtonText: String(body.appIconPopupButtonText ?? current.appIconPopupButtonText ?? "").trim(),
    homeEventsDescription: String(body.homeEventsDescription ?? current.homeEventsDescription ?? "").trim(),
    homeGamesDescription: String(body.homeGamesDescription ?? current.homeGamesDescription ?? "").trim(),
    homeExperiencesDescription: String(body.homeExperiencesDescription ?? current.homeExperiencesDescription ?? "").trim(),
    homeAboutDescription: String(body.homeAboutDescription ?? current.homeAboutDescription ?? "").trim(),
    advertiseWithUsDescription: String(body.advertiseWithUsDescription ?? current.advertiseWithUsDescription ?? "").trim(),
    advertiseWithUsEmail: String(body.advertiseWithUsEmail ?? current.advertiseWithUsEmail ?? "").trim(),
    globalBingoIframeUrl: String(body.globalBingoIframeUrl ?? current.globalBingoIframeUrl ?? "").trim(),
    globalTriviaIframeUrl: String(body.globalTriviaIframeUrl ?? current.globalTriviaIframeUrl ?? "").trim(),
    globalMysteryIframeUrl: String(body.globalMysteryIframeUrl ?? current.globalMysteryIframeUrl ?? "").trim(),
    globalVoteIframeUrl: String(body.globalVoteIframeUrl ?? current.globalVoteIframeUrl ?? "").trim(),
    socialWallOutputIframeUrl: String(body.socialWallOutputIframeUrl ?? current.socialWallOutputIframeUrl ?? "").trim(),
    socialWallInputIframeUrl: String(body.socialWallInputIframeUrl ?? current.socialWallInputIframeUrl ?? "").trim(),
    karaokeUrl: String(body.karaokeUrl ?? current.karaokeUrl ?? "").trim(),
    escapeRoomUrl: String(body.escapeRoomUrl ?? current.escapeRoomUrl ?? "").trim(),
    golfSimulatorUrl: String(body.golfSimulatorUrl ?? current.golfSimulatorUrl ?? "").trim(),
    meetMingleUrl: String(body.meetMingleUrl ?? current.meetMingleUrl ?? "").trim(),
    jobsUrl: String(body.jobsUrl ?? current.jobsUrl ?? "").trim(),
    barflyTvIframeUrl: String(body.barflyTvIframeUrl ?? current.barflyTvIframeUrl ?? "").trim(),
    barflyTvEmptyMessage: String(body.barflyTvEmptyMessage ?? current.barflyTvEmptyMessage ?? "").trim(),
...
function normalizeSponsor(body = {}, existing = {}) {
  return {
    id: existing.id || nanoid(),
    name: String(body.name ?? existing.name ?? "").trim(),
    logoUrl: String(body.logoUrl ?? existing.logoUrl ?? "").trim(),
    websiteUrl: String(body.websiteUrl ?? existing.websiteUrl ?? "").trim(),
    instagramUrl: String(body.instagramUrl ?? existing.instagramUrl ?? "").trim(),
    offerTitle: String(body.offerTitle ?? existing.offerTitle ?? "").trim(),
    offerDetails: String(body.offerDetails ?? existing.offerDetails ?? "").trim(),
...
function publicSponsor(sponsor) {
  ...
  return {
    ...
    offerTitle: sponsor.offerTitle || "",
    offerDetails: sponsor.offerDetails || "",
    redemptionRules: sponsor.redemptionRules || "",
    expirationDate: sponsor.expirationDate || "",
    category: sponsor.category || "",
    active: sponsor.active !== false
  };
}

function normalizeSponsorIds(value = [], existing = []) {
  const source = Array.isArray(value) ? value : String(value || "").split(",");
  const ids = source.map(item => String(item || "").trim()).filter(Boolean);
  const unique = Array.from(new Set(ids));
  const known = new Set((db.sponsors || []).map(sponsor => sponsor.id));
  const filtered = unique.filter(id => known.has(id));
  if (filtered.length || Array.isArray(value)) return filtered.slice(0, 12);
  return Array.isArray(existing) ? existing.filter(id => known.has(String(id))).slice(0, 12) : [];
}

function sponsorsForEvent(event = {}) {
  const sponsorIds = normalizeSponsorIds(event.sponsorIds || [], []);
  const byId = new Map((db.sponsors || []).filter(sponsor => sponsor.active !== false).map(sponsor => [sponsor.id, sponsor]));
  const attached = sponsorIds.map(id => publicSponsor(byId.get(id))).filter(Boolean);

  if (!attached.length && (event.sponsorName || event.sponsorLogoUrl)) {
    attached.push({
...
function normalizeMixerGoal(value) {
  const safe = String(value || "").trim();
  return ["social", "professional", "dating", "marriage", "custom"].includes(safe) ? safe : "";
}

function normalizeEventTypeTags(value = []) {
  const source = Array.isArray(value) ? value : String(value || "").split(",");
  return Array.from(new Set(source.map(item => String(item || "").trim()).filter(Boolean))).slice(0, 12);
}
...
function normalizeHeroTemplate(body = {}, existing = {}) {
  return {
    ...
    heroFit: normalizeHeroFit(body.heroFit ?? existing.heroFit ?? "cover"),
    active: Boolean(body.active ?? existing.active ?? true),
    notes: String(body.notes ?? existing.notes ?? "").trim(),
    createdAt: existing.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString()
...
function normalizeWeeklyEvent(body = {}, existing = {}) {
  ...
  return {
    ...
    heroImageUrl: String(body.heroImageUrl ?? existing.heroImageUrl ?? "").trim(),
    heroFit: normalizeHeroFit(body.heroFit ?? existing.heroFit ?? "cover"),
    showHeroGraphic: Boolean(body.showHeroGraphic ?? existing.showHeroGraphic ?? true),
    eventTypeTags: normalizeEventTypeTags(body.eventTypeTags ?? existing.eventTypeTags ?? []),
    mixerGoal: normalizeMixerGoal(body.mixerGoal ?? existing.mixerGoal ?? ""),
    ...
    daysOfWeek: normalizeDaysOfWeek(body.daysOfWeek ?? existing.daysOfWeek ?? [body.dayOfWeek ?? existing.dayOfWeek ?? new Date().getDay()], body.dayOfWeek ?? existing.dayOfWeek ?? new Date().getDay()),
    ...
    sponsorIds: normalizeSponsorIds(body.sponsorIds ?? existing.sponsorIds ?? [], existing.sponsorIds ?? []),
    ...
    paymentLink: String(body.paymentLink ?? existing.paymentLink ?? "").trim(),
    youtubeShortUrl: String(body.youtubeShortUrl ?? existing.youtubeShortUrl ?? "").trim(),
    tiktokUrl: String(body.tiktokUrl ?? existing.tiktokUrl ?? "").trim(),
    instagramPostUrl: String(body.instagramPostUrl ?? existing.instagramPostUrl ?? "").trim(),
...
function syncMeetMingleSessionForEvent(event) {
  ...
  db.games[id] = {
    ...
    mixerGoal: normalizeMixerGoal(event.mixerGoal || ""),
...
function normalizeDaysOfWeek(value, fallbackDay = 1) {
  const source = Array.isArray(value) ? value : String(value ?? "").split(",");
  const days = source
    .map(day => Number(day))
  ...
  const fallback = Number(fallbackDay);
  return [Number.isInteger(fallback) && fallback >= 0 && fallback <= 6 ? fallback : 1];
}

function eventOccurrenceDate(event, dayOverride = null) {
  if (!event.recurringWeekly && event.date) {
    const d = new Date(`${event.date}T00:00:00`);
    return Number.isNaN(d.getTime()) ? nextDateForDay(event.dayOfWeek) : d;
  }
  return nextDateForDay(dayOverride ?? event.dayOfWeek);
}

function eventOccurrenceDates(event) {
  if (!event.recurringWeekly && event.date) return [eventOccurrenceDate(event)];
  return normalizeDaysOfWeek(event.daysOfWeek ?? [event.dayOfWeek], event.dayOfWeek).map(day => nextDateForDay(day));
}

function publicEvent(event, occurrenceOverride = null) {
  const occurrence = occurrenceOverride || eventOccurrenceDate(event);
  const heroTemplate = heroTemplateForEvent(event);
  const showHeroGraphic = event.showHeroGraphic !== false;
  const heroImageUrl = showHeroGraphic ? (event.heroImageUrl || heroTemplate?.heroImageUrl || "") : "";
  const heroFit = normalizeHeroFit(event.heroFit || heroTemplate?.heroFit || "cover");
  const dateLabel = occurrence.toISOString().slice(0, 10);
  return {
    ...event,
    showHeroGraphic,
    heroImageUrl,
    ...
    eventTypeTags: normalizeEventTypeTags(event.eventTypeTags || []),
    sponsors: sponsorsForEvent(event),
    eventTypeLabel: event.customEventType || (event.eventTypeTags || []).join(" • ") || eventTypeLabels[event.eventType] || "Event",
    dayLabel: dayNames[occurrence.getDay()],
    dateLabel,
    occurrenceDayOfWeek: occurrence.getDay(),
    occurrenceKey: `${event.id}-${dateLabel}`,
    sortKey: `${dateLabel}T${event.startTime || "00:00"}`,
...
function publicEvents({ includeHidden = false } = {}) {
  return (db.events || [])
    ...
    .flatMap(event => eventOccurrenceDates(event).map(occurrence => publicEvent(event, occurrence)))
...
function buildForecastForGame(game) {
  ...
  return {
    ...
    mixerGoal: normalizeMixerGoal(game.mixerGoal || ""),
    ...
    drinkSpecial: getDrinkSpecial(game),
    appOrder: appOrderForGameMode(getGameMode(game), game.mixerGoal || ""),
    capacity: publicCapacity(game),
    forecast
  };
}
...
function ensureRoundPairings(game, round) {
  ...
  const pairings = groups.map((group, pairingIndex) => {
    ...
    const category = weightedCategoryForGroup(group.players, round, getGameMode(game), game.mixerGoal || "");
...
function defaultCustomEventTypes() {
  return [
    "Themed Trivia",
    "General Trivia",
    "Music Trivia",
...
function normalizeCustomEventTypes(types = []) {
  const list = Array.isArray(types) ? types : [];
  const cleaned = list
    .map(item => String(item || "").trim())
    .filter(Boolean);
  return Array.from(new Set(cleaned)).slice(0, 60);
}

app.get("/api/host/custom-event-types", requireHost, (req, res) => {
  const saved = normalizeCustomEventTypes(db.settings?.customEventTypes || []);
  res.json({ customEventTypes: saved.length ? saved : defaultCustomEventTypes() });
});

app.post("/api/host/custom-event-types", requireHost, (req, res) => {
  const customEventTypes = normalizeCustomEventTypes(req.body?.customEventTypes || []);
  db.settings = { ...defaultSettings(), ...(db.settings || {}), customEventTypes };
  res.json({ success: true, customEventTypes });
});
...
app.get("/api/host/sponsors", requireHost, (req, res) => {
  res.json({ sponsors: (db.sponsors || []).map(publicSponsor).sort((a, b) => a.name.localeCompare(b.name)) });
});

app.post("/api/host/sponsors", requireHost, (req, res) => {
  const sponsor = normalizeSponsor(req.body || {});
  if (!sponsor.name) return res.status(400).json({ error: "Sponsor name is required." });
  db.sponsors = [...(db.sponsors || []), sponsor];
  res.json({ success: true, sponsor: publicSponsor(sponsor), sponsors: (db.sponsors || []).map(publicSponsor) });
});

app.post("/api/host/sponsors/:sponsorId", requireHost, (req, res) => {
  const existing = (db.sponsors || []).find(sponsor => sponsor.id === req.params.sponsorId);
  if (!existing) return res.status(404).json({ error: "Sponsor not found." });
  const updated = normalizeSponsor(req.body || {}, existing);
  if (!updated.name) return res.status(400).json({ error: "Sponsor name is required." });
  db.sponsors = (db.sponsors || []).map(sponsor => sponsor.id === existing.id ? updated : sponsor);
  res.json({ success: true, sponsor: publicSponsor(updated), sponsors: (db.sponsors || []).map(publicSponsor) });
});

app.delete("/api/host/sponsors/:sponsorId", requireHost, (req, res) => {
  const before = (db.sponsors || []).length;
  db.sponsors = (db.sponsors || []).filter(sponsor => sponsor.id !== req.params.sponsorId);
  db.events = (db.events || []).map(event => ({
    ...event,
    sponsorIds: Array.isArray(event.sponsorIds) ? event.sponsorIds.filter(id => id !== req.params.sponsorId) : []
  }));
  res.json({ success: true, deleted: before - db.sponsors.length, sponsors: (db.sponsors || []).map(publicSponsor), events: publicEvents({ includeHidden: true }) });
});
...
app.delete("/api/host/events/:eventId", requireHost, (req, res) => {
  ...
  if (existing.meetMingleGameId && db.games[existing.meetMingleGameId]) {
    db.games[existing.meetMingleGameId].linkedEventId = "";
    db.games[existing.meetMingleGameId].linkedEventSlug = "";
    db.games[existing.meetMingleGameId].linkedEventTitle = "";
  }

  const before = (db.events || []).length;
  db.events = (db.events || []).filter(event => event.id !== req.params.eventId);
  res.json({ success: true, deleted: before - db.events.length, unlinkedMeetMingleSession: Boolean(existing.meetMingleGameId), events: publicEvents({ includeHidden: true }) });
});
...
function cohostSessionSummary(game) {
  const publicData = publicGame(game);
  return {
    id: publicData.id,
    gameCode: publicData.gameCode,
    linkedEventTitle: publicData.linkedEventTitle || "",
    linkedEventSlug: publicData.linkedEventSlug || "",
    venueName: publicData.venueName,
    venueLocation: publicData.venueLocation || "",
    status: publicData.status,
    phaseLabel: publicData.phaseLabel,
    phaseType: publicData.phaseType,
    currentRound: publicData.currentRound,
    gameMode: publicData.gameMode,
    modeLabel: publicData.modeLabel,
    playerCount: publicData.players.length,
    rsvpCount: rsvpsForGame(game.id).length,
    checkedInCount: rsvpsForGame(game.id).filter(rsvp => rsvp.checkedInAt).length,
    openSafetyReportCount: db.reports.filter(r => r.gameId === game.id && r.status !== "reviewed").length,
    capacity: publicCapacity(game),
    meetingSetup: publicMeetingSetup(game),
    drinkSpecial: getDrinkSpecial(game),
...
function checkInRsvpForGame(game, rsvp) {
  if (!game) return { error: "Session not found." };
  if (!rsvp) return { error: "RSVP not found." };
  updateGameClock(game);

  if (game.id !== rsvp.gameId) return { error: "This RSVP does not belong to this session." };
  if (game.status !== "lobby") return { error: "This session has already started. Check-in is locked." };

  if (rsvp.playerId && db.players[rsvp.playerId]) {
    return { player: db.players[rsvp.playerId] };
  }

  if (getCapacityStatus(game).checkinFull) return { error: "This session is full for check-ins." };
  ...
  if (existingAlias) return { error: "This alias is already checked in. Ask the player to use a different alias." };
  ...
  game.playerIds = Array.from(new Set([...(game.playerIds || []), id]));
  rsvp.playerId = id;
...
app.post("/api/cohost/login", (req, res) => {
  const pin = String(req.body?.pin || "");
  if (pin !== COHOST_PIN && pin !== HOST_PIN) return res.status(401).json({ error: "Invalid co-host PIN" });
  res.json({ success: true });
});

app.get("/api/cohost/sessions", requireCohost, (req, res) => {
  const sessions = Object.values(db.games)
    .filter(isActiveOrUpcomingGame)
    .map(cohostSessionSummary)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  res.json({ sessions });
});

app.get("/api/cohost/games/:gameId", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  res.json(publicGame(game));
});

app.get("/api/cohost/games/:gameId/rsvps", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  updateGameClock(game);
  res.json({
...
app.get("/api/cohost/games/:gameId/reports", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  const reports = db.reports
    ...
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .map(publicReport);
  res.json({ gameId: game.id, reportCount: reports.length, reports });
});

app.post("/api/cohost/games/:gameId/start-game", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  if (game.status !== "lobby") return res.status(400).json({ error: "Reset this session before starting again." });
...
app.post("/api/cohost/games/:gameId/pause", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  updateGameClock(game);
  if (["lobby", "complete"].includes(game.status)) return res.status(400).json({ error: "Session is not running." });
...
app.post("/api/cohost/games/:gameId/resume", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  if (game.status !== "paused" || !game.pausedAt) return res.status(400).json({ error: "Session is not paused." });
...
app.post("/api/cohost/games/:gameId/skip-phase", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  if (!game.startedAt || game.status === "complete") return res.status(400).json({ error: "No active phase to advance." });
  if (game.status === "paused") return res.status(400).json({ error: "Resume before advancing." });
  const elapsed = Math.floor((Date.now() - new Date(game.startedAt).getTime()) / 1000);
  const timeline = getGameTimeline(game);
...
app.post("/api/cohost/games/:gameId/end-game", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
...
app.post("/api/cohost/games/:gameId/checkin-rsvp", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  const rsvp = (db.rsvps || []).find(item => item.id === req.body.rsvpId);
  const result = checkInRsvpForGame(game, rsvp);
  if (result.error) return res.status(400).json({ error: result.error });
  res.json({ success: true, player: result.player, rsvp: publicRsvp(rsvp), game: publicGame(game) });
});

app.post("/api/cohost/rsvps/:rsvpId/cancel", requireCohost, (req, res) => {
  const rsvp = (db.rsvps || []).find(item => item.id === req.params.rsvpId);
  if (!rsvp) return res.status(404).json({ error: "RSVP not found" });
  const game = db.games[rsvp.gameId];
  updateGameClock(game);
  if (rsvp.checkedInAt) return res.status(400).json({ error: "This RSVP is already checked in. Remove the player instead." });
  rsvp.status = "cancelled";
  rsvp.updatedAt = new Date().toISOString();
  res.json({ success: true, rsvp: publicRsvp(rsvp) });
});

app.post("/api/cohost/rsvps/:rsvpId/no-show", requireCohost, (req, res) => {
  const rsvp = (db.rsvps || []).find(item => item.id === req.params.rsvpId);
  if (!rsvp) return res.status(404).json({ error: "RSVP not found" });
  if (rsvp.checkedInAt) return res.status(400).json({ error: "This guest is already checked in." });
  rsvp.status = "no_show";
  rsvp.updatedAt = new Date().toISOString();
  res.json({ success: true, rsvp: publicRsvp(rsvp) });
});

app.post("/api/cohost/games/:gameId/remove-player", requireCohost, (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Session not found" });
  ...
  const rsvp = (db.rsvps || []).find(item => item.playerId === player.id);
  if (rsvp) {
    rsvp.status = "removed";
    rsvp.updatedAt = new Date().toISOString();
  }
  res.json({ success: true, game: publicGame(game) });
});
...
app.post("/api/games", requireHost, (req, res) => {
  ...
  db.games[id] = {
    ...
    mixerGoal: normalizeMixerGoal(req.body.mixerGoal || ""),
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v126 running on port ${PORT}`));
}).catch(err => {
