...
app.get("/api/games/:gameId/reports", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
...
app.post("/api/games/:gameId/remove-player", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
...
app.post("/api/reports/:reportId/review", (req, res) => {
...
app.listen(PORT, () => console.log(`BARFLY DATE v9 running on port ${PORT}`));
