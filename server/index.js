...
function normalizeWeeklyEvent(body = {}, existing = {}) {
  ...
  return {
    ...
    socialWallIframeUrl: String(body.socialWallIframeUrl ?? existing.socialWallIframeUrl ?? "").trim(),
    bingoIframeUrl: String(body.bingoIframeUrl ?? existing.bingoIframeUrl ?? "").trim(),
    triviaIframeUrl: String(body.triviaIframeUrl ?? existing.triviaIframeUrl ?? "").trim(),
    mysteryIframeUrl: String(body.mysteryIframeUrl ?? existing.mysteryIframeUrl ?? "").trim(),
    voteIframeUrl: String(body.voteIframeUrl ?? existing.voteIframeUrl ?? "").trim(),
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v93 running on port ${PORT}`));
}).catch(err => {
