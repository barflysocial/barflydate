...
function normalizeSettings(body = {}) {
  ...
  return {
    ...
    eventCities: String(body.eventCities ?? current.eventCities ?? "Baton Rouge").trim() || "Baton Rouge",
...
function normalizeWeeklyEvent(body = {}, existing = {}) {
  ...
  return {
    ...
    description: String(body.description ?? existing.description ?? "").trim(),
    prizeSpecial: String(body.prizeSpecial ?? existing.prizeSpecial ?? "").trim(),
    instagramCaption: String(body.instagramCaption ?? existing.instagramCaption ?? "").trim(),
    shortPromoText: String(body.shortPromoText ?? existing.shortPromoText ?? "").trim(),
    hashtags: String(body.hashtags ?? existing.hashtags ?? "").trim(),
    postNotes: String(body.postNotes ?? existing.postNotes ?? "").trim(),
...
initPersistence().then(() => {
  app.listen(PORT, () => console.log(`BARFLY DATE v99 running on port ${PORT}`));
}).catch(err => {
