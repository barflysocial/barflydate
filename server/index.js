...
const { weightedCategoryForGroup, promptsFor, sharedInterests } = require("./prompts");
...
function buildTimeline() {
  ...
  phases.push({ type: "voting", round: 10, label: "Round 10: Moment Voting", startOffset: t, endOffset: t + FLOW.votingSeconds });
...
function activePlayersForRound(game, round) {
  ...
  return getGamePlayers(game).filter(player => {
    ...
    return ageSeconds <= FLOW.inactiveAfterSeconds;
  });
}
...
function publicGame(game) {
  ...
  return {
    ...game,
    flow: FLOW,
    totalGameSeconds: TIMELINE.totalSeconds,
    voteCategories,
...
function ensureRoundPairings(game, round) {
  if (!round || round < 1 || round > FLOW.totalRounds) return;
  ...
  const pairings = groups.map(group => {
    ...
    const category = weightedCategoryForGroup(group.players, round);
    ...
    const pairing = {
      ...
      categoryReason: shared.includes(category) ? "Shared spark" : "Discovery spark",
...
app.post("/api/games/:gameId/start-game", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
...
app.post("/api/games/:gameId/pause", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
  updateGameClock(game);
...
app.post("/api/games/:gameId/resume", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
...
app.post("/api/games/:gameId/skip-phase", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
...
app.post("/api/games/:gameId/end-game", (req, res) => {
  const game = db.games[req.params.gameId];
  if (!game) return res.status(404).json({ error: "Game not found" });
  game.startedAt = iso(Date.now() - VOTING_OFFSET * 1000);
  updateGameClock(game);
  res.json(publicGame(game));
});
...
app.post("/api/join", (req, res) => {
  ...
  if (["voting", "complete"].includes(game.status)) {
    return res.status(400).json({ error: "This game is in voting/results. Ask the host for a new code." });
  }

  const id = nanoid();
  const eligibleRound = game.status === "lobby" || game.status === "intro"
    ? 1
    : Math.min((game.currentRound || 0) + 1, FLOW.totalRounds + 1);
  ...
  const player = {
    ...
    eligibleRound,
    nickname: req.body.nickname || "The Secret Admirer",
    ...
    lookingFor: req.body.lookingFor || "social",
...
app.get("/api/player/:playerId/connections", (req, res) => {
  ...
  const mine = db.decisions.filter(d => d.gameId === game.id && d.fromPlayerId === player.id && ["yes", "maybe"].includes(d.decision) && ["definitely", "open"].includes(d.outsideChoice));
  const connections = [];
  ...
  for (const a of mine) {
    ...
    const reasons = [];
    if (history.sharedInterests.length) reasons.push(`${history.sharedInterests.length} shared spark${history.sharedInterests.length === 1 ? "" : "s"}: ${history.sharedInterests.join(", ")}`);
    if (score > 0) reasons.push(`${score} reaction point${score === 1 ? "" : "s"} between you`);
  ...
  res.json({ player, game: publicGame(game), connections });
});
...
app.listen(PORT, () => console.log(`BARFLY DATE v5 running on port ${PORT}`));
