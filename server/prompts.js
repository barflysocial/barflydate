...
function weightedCategoryForGroup(players, round) {
  const shared = sharedInterests(players);
  const desiredTier = tierForRound(round);
  const tierCategories = categories.filter(c => promptLibrary[c].tier === desiredTier);

  // 65%: shared-interest category when available.
  if (shared.length > 0 && Math.random() < 0.65) {
    return randomFrom(shared);
  }

  // 25%: emotional arc category based on early/mid/deep round pacing.
  if (Math.random() < 0.75) {
    return randomFrom(tierCategories);
  }

  // 10%: full discovery category from all 9 apps.
