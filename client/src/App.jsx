...
function BusinessContactButtons({ settings, compact = false }) {
  ...
  return <div className={compact ? "businessButtonGrid compactButtons" : "businessButtonGrid"}>
    <Button onClick={() => openBusinessLink(booking)}>Book an Event</Button>
...
function eventPrimaryAction(event) {
  ...
  return {
    label: eventUnavailable(event)
      ...
      : (event?.paidEvent ? paidLabel : (event?.buttonText || (isBarflySocial ? "View Meet & Mingle / RSVP" : isOnline ? "Play Now" : "More Info"))),
    link: event?.paidEvent ? (event?.paymentLink || event?.buttonLink || event?.publicPath || `/events/${event?.slug || event?.id}`) : (event?.buttonLink || (isBarflySocial ? "/forecast" : (event?.publicPath || `/events/${event?.slug || event?.id}`)))
  };
}
...
const APP_VERSION = "BARFLYDATE v71";
...
function QrDisplay() {
  ...
  const labels = {
    ...
    book: { title: "Scan to Book", subtitle: "Request a Barfly Entertainment event." },
    booking: { title: "Scan to Book", subtitle: "Request a Barfly Entertainment event." },
      ...
      {viewMode === "month" && <div className="calendarModePanel">
        <div className="calendarHeader">
          ...
          <div>
            ...
            <p className="muted">Tap a date to view events.</p>
          </div>
          <Button className="secondary" onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 1))}>›</Button>
        </div>
...
function BookingPage() {
  ...
  async function submitBooking() {
    ...
    setSaved("Thank you! Your booking request has been sent. Barfly Entertainment will review your event details and contact you soon.");
    setSubmitted(true);
    setForm({
      name: "",
      eventScope: "public",
  ...
  return <div className="screen publicScreen neonPublic bookingPage">
    ...
    <div className="publicHero compactHero">
      <div className="brandMark">BOOK BARFLY ENTERTAINMENT</div>
      <h1>Request an Event</h1>
      <p className="tagline">Tell us what you want to book and how to reach you.</p>
    </div>
    ...
    {submitted && <div className="card neonCard bookingThanksCard">
      ...
      <p>Barfly Entertainment will review your event details and contact you soon.</p>
    ...
    {!submitted && <div className="card neonCard bookingCard">
      ...
      <div className="ctaRow">
        <Button onClick={submitBooking}>Send Booking Request</Button>
        <Button className="secondary" onClick={() => window.location.href = "/events"}>View Events</Button>
      </div>

      <p className="microcopy">You can also contact Barfly Entertainment directly:</p>
...
function HostQuickActions() {
  ...
  return <div className="card glowCard hostQuickActions" id="host-quick-actions">
    ...
    <div className="quickActionGrid">
      ...
      <Button className="secondary" onClick={() => openSection("host-section-events", "host-booking-calendar")}>Booking Calendar</Button>
      <Button className="secondary" onClick={() => openSection("host-section-events", "host-bookings")}>Booking List</Button>
      <Button className="secondary" onClick={() => openSection("host-section-business", "host-analytics")}>View Analytics</Button>
      <Button className="secondary" onClick={() => openSection("host-section-gameplay", "create-social-session")}>Create Meet & Mingle Session</Button>
...
function HostTemplateManager() {
  ...
  return <div className="card glowCard hostTemplateManager" id="host-templates">
    ...
    <div className="twoCol">
      ...
      <div><label>Display Event Type</label><input value={form.customEventType} onChange={e => update("customEventType", e.target.value)} placeholder="Digital Mystery, 80’s Trivia, etc." /></div>
      <div><label>Default Title</label><input value={form.title} onChange={e => update("title", e.target.value)} /></div>
      <div><label>Primary Button Name</label><input value={form.buttonText} onChange={e => update("buttonText", e.target.value)} /></div>
      <div><label>Primary Button Link</label><input value={form.buttonLink} onChange={e => update("buttonLink", e.target.value)} /></div>
...
function BusinessDemo() {
  ...
  return <div className="screen publicScreen neonPublic businessDemo">
    ...
    <div className="businessHero">
      ...
      <div className="ctaRow">
        <Button onClick={() => window.location.href = "/book"}>Request Event Booking</Button>
    ...
    <div id="business-contact" className="card contactCard">
      ...
      <BusinessContactButtons settings={settings} />
      <div className="ctaRow">
        <Button onClick={() => window.location.href = "/book"}>Request Event Booking</Button>
        <Button className="secondary" onClick={() => window.location.href = "/events"}>View Events Calendar</Button>
      </div>
...
function HostEventsPanel() {
  ...
  function applyMatchingHero() {
    const venueKey = slugifyText(form.venueName || "");
    const match = heroTemplates.find(template =>
      template.active !== false &&
      slugifyText(template.venueName || "") === venueKey &&
      template.heroVariant === form.heroVariant &&
      template.heroImageUrl
    );
    if (!match) {
      alert("No matching venue hero saved for this venue + variant yet.");
      return;
    }
    setForm(current => ({ ...current, heroImageUrl: match.heroImageUrl }));
  }
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
    ...
    <div className="primaryActionPreview">Public button preview: <b>{form.buttonText || (form.eventType === "barfly_social" ? "View Meet & Mingle / RSVP" : form.eventType === "escape_room" ? "Start Escape Room" : form.eventType === "mystery" ? "Start Mystery" : form.onlineOnly ? "Play Now" : "More Info")}</b></div>
...
function HostManager() {
  ...
  return <div className="screen hostManagerCompact">
    ...
    <HostSection id="host-section-gameplay" title="Gameplay" subtitle="Create sessions, test games, active games, and run-of-show.">
      ...
      <h2>Create New Meet & Mingle Session</h2>
