...
const APP_VERSION = "BARFLYDATE v91";
  ...
  return <div className="screen publicScreen neonPublic socialWallPage">
    ...
    <div className="socialWallFrameGrid">
      {frames.map(frame => <div className="card neonCard socialWallFrameCard" key={frame.url}>
        ...
        <h2>{frame.title}</h2>
        <iframe src={frame.url} title={frame.title} loading="lazy" allow="camera; microphone; clipboard-write; fullscreen" />
        <p className="microcopy">If this frame does not load, open it directly.</p>
        <Button className="secondary" onClick={() => window.location.href = frame.url}>Open Direct</Button>
      </div>)}
    </div>
