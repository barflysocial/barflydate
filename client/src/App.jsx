...
const APP_VERSION = "BARFLYDATE v79";
...
function HostQuickActions() {
  ...
  return <div className="card glowCard hostQuickActions guidedHostFlows" id="host-quick-actions">
    ...
    <div className="guidedActionGrid">
      <button type="button" onClick={() => openSection("host-section-events", "host-events")}><b>➕ Create Event</b><span>Type, venue, time, hero, Meet & Mingle, save.</span></button>
      <button type="button" onClick={() => openSection("host-section-business", "host-venues")}><b>🏟️ Add Venue</b><span>Save venue details, logo, contact info, and defaults.</span></button>
      <button type="button" onClick={() => openSection("host-section-marketing", "host-hero-templates")}><b>🖼️ Create Hero Graphic</b><span>Choose venue, event type, image URL, fit, preview, save.</span></button>
      ...
      <button type="button" onClick={() => openSection("host-section-marketing", "host-qr-tools")}><b>▣ Generate QR Code</b><span>Events, demo, Meet & Mingle, check-in, or request page.</span></button>
      <button type="button" onClick={() => openSection("host-section-events", "host-bookings")}><b>📨 Review Requests</b><span>Review booking requests and update status.</span></button>
  ...
  return <div className="screen publicScreen neonPublic businessDemo">
    ...
    <div id="business-contact" className="card contactCard demoBottomActions">
      ...
      <div className="ctaRow demoPrimaryActions">
        ...
        <Button onClick={() => window.location.href = "/qr/demo"}>QR Code</Button>
      </div>
...
function HostEventsPanel() {
  ...
  const heroPreviewUrl = form.heroImageUrl || suggestedHeroUrl || form.eventImageUrl || "";
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
      ...
      <div className="wizardStep">
        ...
        <div>
          ...
          <div className="heroTemplateGallery">
            ...
            <h4>Choose from saved hero previews</h4>
            {activeHeroTemplates.length === 0 && <p className="muted">No saved hero templates with image URLs yet.</p>}
            {activeHeroTemplates.length > 0 && <div className="heroGalleryGrid">
              {activeHeroTemplates.map(template => <button type="button" key={template.id} className={form.heroImageUrl === template.heroImageUrl ? "heroGalleryCard selected" : "heroGalleryCard"} onClick={() => useHeroTemplate(template)}>
                ...
                <span>{heroVariantLabel(template.heroVariant)}</span>
          ...
          <div className="heroGraphicPreviewWindow">
            ...
            <div className={heroPreviewUrl ? "heroGraphicPreview hasImage" : "heroGraphicPreview"} style={heroPreviewUrl ? { backgroundImage: `linear-gradient(180deg, rgba(0,0,0,.08), rgba(0,0,0,.34)), url(${heroPreviewUrl})`, backgroundSize: heroFit, backgroundRepeat: "no-repeat", backgroundPosition: "center" } : undefined}>
              {!heroPreviewUrl && <div className="eventTemplateFallback">{heroVariantLabel(form.heroVariant)}</div>}
            ...
            <p className="microcopy">This is the 4:5 hero area used on the event card. Use 1080×1350 or 2160×2700 hero graphics. Current fit: {heroFit === "contain" ? "Contain / show full image" : "Cover / fill box"}.</p>
    ...
    <details className="advancedEventDetails">
      ...
      <div><label>Hero Image URL</label><input value={form.heroImageUrl} onChange={e => update("heroImageUrl", e.target.value)} placeholder="Auto-filled from Venue + Hero Variant" /><Button className="secondary miniButton" onClick={applyMatchingHero}>Use Matching Venue Hero</Button>{heroUrlWarning(form.heroImageUrl) && <p className="warningText">{heroUrlWarning(form.heroImageUrl)}</p>}</div>
...
function HostManager() {
  ...
  return <div className="screen hostManagerCompact">
    ...
    <HostSection id="host-section-events" title="Events & Calendar" subtitle="Event calendar, booking requests, and event editor.">
      <HostEventCalendar />
      <HostBookingsPanel />
      <HostEventsPanel />
    </HostSection>

    <HostSection id="host-section-marketing" title="Marketing" subtitle="Venue hero templates, event templates, QR tools, and promo assets.">
...
function CoHostDashboard() {
  ...
  async function runAction(path, successText, body = null) {
    setMessage("");
    setError("");
  ...
  return <div className="screen hostManagerCompact cohostDashboard">
    ...
    {game && <div className="card glowCard cohostStep">
      ...
      <div className="ctaRow">
        ...
        {game.status !== "complete" && <Button className="danger" onClick={() => runAction(`/cohost/games/${game.id}/end-game`, "Session moved to reveal/end.")}>End Session</Button>}
      </div>
    </div>}
    ...
    {game && <div className="card glowCard cohostStep">
      ...
      {filteredRsvps().map(rsvp => <div className="rsvpRow" key={rsvp.id}>
        ...
        <div className="hostPlayerActions playerViewActions">
          ...
          {!rsvp.checkedIn && <Button className="secondary" onClick={() => runAction(`/cohost/rsvps/${rsvp.id}/no-show`, `${rsvp.nickname} marked no-show.`)}>No-Show</Button>}
          {!rsvp.checkedIn && <Button className="danger" onClick={() => runAction(`/cohost/rsvps/${rsvp.id}/cancel`, `${rsvp.nickname} cancelled.`)}>Cancel</Button>}
        </div>
      </div>)}
    </div>}
    ...
    {game && <div className="card glowCard cohostStep">
      ...
      {(game.players || []).filter(player => !player.removedAt).map(player => <div className="rsvpRow" key={player.id}>
        ...
        <div className="hostPlayerActions playerViewActions">
          <Button className="danger" onClick={() => runAction(`/cohost/games/${game.id}/remove-player`, `${player.nickname} removed.`, { playerId: player.id })}>Remove Player</Button>
        </div>
      </div>)}
