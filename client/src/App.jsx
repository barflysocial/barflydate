...
function usePublicSettings() {
  const [settings, setSettings] = React.useState({
    ...
    homeEventsDescription: "Create, publish, and manage public or private Barfly Social events.",
    homeGamesDescription: "Structured game formats for trivia, bingo, mystery, and escape room events.",
    homeExperiencesDescription: "Interactive add-ons for social engagement, voting, karaoke, golf, live screens, and Barfly TV.",
    homeAboutDescription: "Learn about Barfly Social, job opportunities, Instagram, advertise with us, and contact options.",
    advertiseWithUsDescription: "Put your business in front of Barfly Social players with event sponsorships, in-app promo placements, Barfly TV ads, social wall features, prize partnerships, and custom game-night activations. Send us your business name, contact info, goals, preferred dates, and budget range to apply.",
    advertiseWithUsEmail: BUSINESS_EMAIL,
...
function eventSocialLinks(event = {}) {
  return [
    { key: "youtubeShortUrl", label: "YouTube Short", icon: "▶", url: event.youtubeShortUrl },
    { key: "tiktokUrl", label: "TikTok", icon: "♪", url: event.tiktokUrl },
    { key: "instagramPostUrl", label: "Instagram", icon: "◎", url: event.instagramPostUrl }
  ].map(item => ({ ...item, href: normalizeExternalUrl(item.url) })).filter(item => item.href);
}

function EventSocialButtons({ event }) {
  const links = eventSocialLinks(event);
  if (!links.length) return null;
  return <div className="eventSocialButtonRow">
    {links.map(link => <Button key={link.key} className="secondary eventSocialButton" onClick={() => {
      trackEventMetric("social_promo_click", event, { platform: link.key, link: link.href });
...
function eventSponsorList(event = {}) {
  const sponsors = Array.isArray(event.sponsors) ? event.sponsors.filter(sponsor => sponsor && sponsor.name) : [];
  if (sponsors.length) return sponsors;
  if (event.sponsorName || event.sponsorLogoUrl) {
    return [{
      id: `legacy-${event.id || "sponsor"}`,
      name: event.sponsorName || "Sponsor",
      logoUrl: event.sponsorLogoUrl || "",
      websiteUrl: "",
      instagramUrl: "",
      offerTitle: event.prizeName || event.prizeSpecial || "",
      offerDetails: event.prizeValue || "",
...
function EventSponsorsDisplay({ event, compact = false }) {
  const sponsors = eventSponsorList(event);
  if (!sponsors.length) return null;
  return <div className={compact ? "eventSponsorsShowcase compactSponsorShowcase" : "eventSponsorsShowcase"}>
    <div className="sponsorShowcaseHeader">Sponsored By</div>
    <div className="sponsorLogoGrid">
      {sponsors.map(sponsor => {
        const website = normalizeExternalUrl(sponsor.websiteUrl);
        const instagram = normalizeExternalUrl(sponsor.instagramUrl);
        const clickUrl = website || instagram;
        return <div className="sponsorMiniCard" key={sponsor.id || sponsor.name}>
          {sponsor.logoUrl && <img src={sponsor.logoUrl} alt={sponsor.name} />}
          <div>
            <b>{sponsor.name}</b>
            {sponsor.offerTitle && <span>{sponsor.offerTitle}</span>}
            {!compact && sponsor.offerDetails && <small>{sponsor.offerDetails}</small>}
            {!compact && sponsor.redemptionRules && <small>{sponsor.redemptionRules}</small>}
            {!compact && sponsor.expirationDate && <small>Expires: {sponsor.expirationDate}</small>}
            {clickUrl && <button type="button" onClick={() => {
              trackEventMetric("sponsor_click", event, { sponsor: sponsor.name, link: clickUrl });
...
function eventSponsorNames(event = {}) {
  return eventSponsorList(event).map(sponsor => sponsor.name).filter(Boolean);
}
...
function eventInstagramCaption(event, settings = {}) {
  ...
  const lines = [
    ...
    eventSponsorNames(event).length ? `Sponsored by ${eventSponsorNames(event).join(", ")}` : "",
    "",
    `${action.label}:`,
...
function eventPlayLink(event) {
  const raw = String(event?.buttonLink || "").trim();
  if (!raw) return "";
  const publicPath = String(event?.publicPath || "").trim();
  const defaultPaths = new Set(["/events", publicPath, `/events/${event?.slug || event?.id}`]);
  if (defaultPaths.has(raw)) return "";
  return normalizeExternalUrl(raw);
}

function eventHasPlayLink(event) {
  return Boolean(eventPlayLink(event));
}

function openEventPlayLink(event) {
  const link = eventPlayLink(event);
  if (!link) return;
  trackEventMetric("play_click", event, { label: "Play", link, type: "event_play_link" });
...
function globalIframeTools(settings = {}) {
  const tools = [
    ...
    { key: "escape-room", label: "Escape Room", icon: "🔐", url: settings.escapeRoomUrl, title: "Barfly Escape Room" },
    { key: "golf-simulator", label: "Golf Simulator", icon: "⛳", url: settings.golfSimulatorUrl, title: "Barfly Golf Simulator" },
    { key: "meet-mingle", label: "Meet & Mingle", icon: "🤝", url: settings.meetMingleUrl, title: "Barfly Meet & Mingle" },
...
const APP_VERSION = "BARFLYDATE v126";
...
function EventsCalendar() {
  ...
  function eventMatchesCategory(event, category) {
    ...
    const map = {
      bingo: ["bingo", "loteria", "lotería", "blackout", "music bingo"],
      trivia: ["trivia", "themed trivia", "music trivia", "general trivia"],
      karaoke: ["karaoke", "battle karaoke", "karoake"],
      golf: ["golf", "putting", "simulator", "full swing"],
      "meet-mingle": ["meet", "mingle", "mixer", "social", "barfly social", "networking"],
      "escape-room": ["escape", "escape room", "digital escape"],
      mystery: ["mystery", "murder mystery", "detective", "clue"],
      vote: ["vote", "voting", "poll"]
    };
    return (map[category] || [category]).some(term => text.includes(term));
  }
  ...
  const categoryEvents = !urlCategory ? events : events.filter(event => eventMatchesCategory(event, urlCategory));
  const searchedEvents = !search ? categoryEvents : categoryEvents.filter(event => [
    event.title,
    event.venueName,
    event.venueLocation,
    event.eventTypeLabel,
  ...
  const categoryLabels = {
    ...
    golf: "Golf Simulator Events",
    "meet-mingle": "Meet & Mingle Events",
    "escape-room": "Escape Room Events",
    mystery: "Mystery Events",
    vote: "Voting Events"
  };
  const categoryLabel = categoryLabels[urlCategory] || "";
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    ...
    <div className="card neonCard eventCalendarCard">
      <div className="brandMark">{categoryLabel || "THIS WEEK’S EVENTS"}</div>
      ...
      {viewMode === "week" && <div className="calendarModePanel">
        <div className="calendarHeader simpleCalendarHeader">
          <h2>{categoryLabel || "This Week’s Events"}</h2>
      ...
      <div className="eventCardGrid">
        {dayEvents.map(event => {
          ...
          return <div className={`${event.featured ? "card neonCard eventCard premiumEventCard featuredEvent" : "card neonCard eventCard premiumEventCard"} ${event.showHeroGraphic === false ? "noHeroEventCard" : ""}`} key={event.occurrenceKey || event.id}>
            ...
            {eventHasPlayLink(event) && !eventUnavailable(event) && <div className="eventHeroPlayRow">
              <Button onClick={() => openEventPlayLink(event)}>▶ Play</Button>
            </div>}
            ...
            <div className="premiumEventInfo">
              ...
              <EventSponsorsDisplay event={event} compact />
...
function EventDetail() {
  ...
  return <div className="screen publicScreen neonPublic eventDetailPage">
    ...
    <div className="card neonCard">
      ...
      {eventHasPlayLink(event) && !eventUnavailable(event) && <div className="eventHeroPlayRow eventDetailPlayRow">
        <Button onClick={() => openEventPlayLink(event)}>▶ Play</Button>
      </div>}
      ...
      {event.prizeSpecial && <p className="sparkLine">{event.prizeSpecial}</p>}
        {event.paidEvent && <p className="ticketLine">🎟️ {event.ticketPrice || "Ticketed event"}</p>}
      <EventSponsorsDisplay event={event} />
    ...
    <div className={`igPosterCanvas ${posterSize} template-${template?.slug || "game-night"} layout-${overlayLayout}`}>
      ...
      <div className="igPosterOverlay">
        <div className="igBadgeRow">
          <span>{badgeText}</span>
          {eventSponsorNames(event).length > 0 && <span>Sponsored by {eventSponsorNames(event).slice(0, 2).join(" + ")}</span>}
        </div>
        <h1>{event.title}</h1>
...
function HostSponsorManager() {
  ...
  async function loadSponsors() {
    const res = await hostFetch(`${API}/host/sponsors`);
    const data = await res.json().catch(() => ({ sponsors: [] }));
    setSponsors(data.sponsors || []);
  }

  React.useEffect(() => { loadSponsors(); }, []);
  ...
  function editSponsor(sponsor) {
    ...
    setForm({ ...blank, ...sponsor, active: sponsor.active !== false });
  }

  async function saveSponsor() {
    const wasEditing = Boolean(editingId);
    const url = editingId ? `${API}/host/sponsors/${editingId}` : `${API}/host/sponsors`;
    const res = await hostFetch(url, {
      method: "POST",
      headers: hostHeaders(true),
      body: JSON.stringify(form)
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.error) return alert(data.error || "Could not save sponsor.");
    setEditingId("");
    setForm(blank);
    setSponsors(data.sponsors || []);
    setSaveMessage(wasEditing ? "Sponsor updated successfully." : "Sponsor saved successfully.");
    setTimeout(() => setSaveMessage(""), 2200);
  }

  async function deleteSponsor(sponsor) {
    if (!confirm(`Delete this sponsor and remove it from attached events?\n\n${sponsor.name || "Unnamed sponsor"}`)) return;
    const res = await hostFetch(`${API}/host/sponsors/${sponsor.id}`, { method: "DELETE" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.error) return alert(data.error || "Could not delete sponsor.");
    setSponsors(data.sponsors || []);
    setSaveMessage("Sponsor deleted successfully.");
    setTimeout(() => setSaveMessage(""), 2200);
  }

  return <div className="card glowCard hostSponsorManager" id="host-sponsors">
    <div className="brandMark">SPONSOR MANAGER</div>
    <h2>{editingId ? "Edit Sponsor" : "Add Sponsor"}</h2>
    {saveMessage && <div className="successBanner">✅ {saveMessage}</div>}
    <p className="microcopy">Create sponsor profiles once, then attach them to any event from the event builder.</p>
    <div className="twoCol">
      <div><label>Sponsor Name</label><input value={form.name} onChange={e => update("name", e.target.value)} placeholder="Topgolf Baton Rouge" /></div>
      <div><label>Category</label><input value={form.category} onChange={e => update("category", e.target.value)} placeholder="Prize Sponsor, Featured Sponsor, Food Partner" /></div>
      <div><label>Logo URL</label><input value={form.logoUrl} onChange={e => update("logoUrl", e.target.value)} placeholder="https://..." /></div>
      <div><label>Website URL</label><input value={form.websiteUrl} onChange={e => update("websiteUrl", e.target.value)} placeholder="https://..." /></div>
      <div><label>Instagram URL</label><input value={form.instagramUrl} onChange={e => update("instagramUrl", e.target.value)} placeholder="https://instagram.com/..." /></div>
      <div><label>Offer / Prize Title</label><input value={form.offerTitle} onChange={e => update("offerTitle", e.target.value)} placeholder="$15 off gameplay" /></div>
      <div><label>Expiration Date</label><input value={form.expirationDate} onChange={e => update("expirationDate", e.target.value)} placeholder="5/31/2026" /></div>
      <label className="checkRow alignEnd"><input type="checkbox" checked={form.active !== false} onChange={e => update("active", e.target.checked)} /><span>Active sponsor</span></label>
    </div>
    <label>Offer Details</label>
    <textarea value={form.offerDetails} onChange={e => update("offerDetails", e.target.value)} placeholder="Add the main offer, prize, or sponsor message shown on event pages." />
    <label>Redemption Rules</label>
    <input value={form.redemptionRules} onChange={e => update("redemptionRules", e.target.value)} placeholder="Dine-in only, 21+, excludes alcohol, etc." />
    <div className="ctaRow">
      <Button onClick={saveSponsor}>{editingId ? "Update Sponsor" : "Save Sponsor"}</Button>
      {editingId && <Button className="secondary" onClick={() => { setEditingId(""); setForm(blank); }}>Cancel Edit</Button>}
    </div>

    <h3>Saved Sponsors</h3>
    {sponsors.length === 0 && <p className="muted">No sponsors saved yet.</p>}
    {sponsors.map(sponsor => <div className="rsvpRow sponsorAdminRow" key={sponsor.id}>
      <div className="sponsorAdminInfo">
        {sponsor.logoUrl && <img src={sponsor.logoUrl} alt={sponsor.name} />}
        <div>
          <b>{sponsor.name}</b>
          <p className="muted">{[sponsor.category, sponsor.offerTitle].filter(Boolean).join(" • ") || "No offer listed"}</p>
          <p className="microcopy">{sponsor.websiteUrl || "No website"} {sponsor.instagramUrl ? `• ${sponsor.instagramUrl}` : ""}</p>
          {sponsor.active === false && <p className="microcopy storageWarnText">Inactive</p>}
        </div>
      </div>
      <div className="hostPlayerActions playerViewActions">
        <Button className="secondary" onClick={() => editSponsor(sponsor)}>Edit</Button>
        <Button className="danger" onClick={() => deleteSponsor(sponsor)}>Delete</Button>
      </div>
    </div>)}
  </div>;
}
...
function HomeNavButtons({ backHref = "/home", backLabel = "Back", showHome = true }) {
  return <div className="hierarchyNavRow">
    <Button className="secondary" onClick={() => window.location.href = backHref}>← {backLabel}</Button>
    {showHome && <Button className="secondary" onClick={() => window.location.href = "/home"}>Home</Button>}
  </div>;
}
...
function HomePage() {
  ...
  const descriptions = {
    events: settings?.homeEventsDescription || "Create, publish, and manage public or private Barfly Social events.",
    games: settings?.homeGamesDescription || "Structured game formats for trivia, bingo, mystery, and escape room events.",
    experiences: settings?.homeExperiencesDescription || "Interactive add-ons for social engagement, voting, karaoke, golf, live screens, and Barfly TV.",
    about: settings?.homeAboutDescription || "Learn about Barfly Social, job opportunities, Instagram, and contact options."
  };
  ...
  function renderMenu({ mark, title, description, items, backHref = "/home", backLabel = "Back", showHome = true }) {
    return <div className="screen publicScreen neonPublic homeLauncherPage hierarchyPage">
      <PublicBrandHeader />
      <HomeNavButtons backHref={backHref} backLabel={backLabel} showHome={showHome} />
      <div className="homeLauncherHero hierarchyHero">
        <div className="brandMark">{mark}</div>
        <h1>{title}</h1>
        {description && <p className="tagline hierarchyDescription">{description}</p>}
      </div>
      <div className="appIconGrid hierarchyIconGrid">
        {items.map(item => <button type="button" key={item.label} className="appIconTile homeActionTile hierarchyTile" onClick={() => openTile(item)}>
          <span className="appIconEmoji">{item.icon}</span>
          <b>{item.label}</b>
          {item.text && <small>{item.text}</small>}
        </button>)}
      </div>
      {pendingTile && renderPopup()}
    </div>;
  }

  function renderLeaf({ mark, title, description, items, backHref, backLabel }) {
    return <div className="screen publicScreen neonPublic homeLauncherPage hierarchyPage">
      <PublicBrandHeader />
      <HomeNavButtons backHref={backHref} backLabel={backLabel} showHome />
      <div className="homeLauncherHero hierarchyHero">
        <div className="brandMark">{mark}</div>
        <h1>{title}</h1>
        {description && <p className="tagline hierarchyDescription">{description}</p>}
      </div>
      <div className="hierarchyLeafGrid">
        {items.map(item => <div className="gameCategoryCard hierarchyLeafCard" key={item.label}>
          <div className="gameCategoryHeader">
            <span className="appIconEmoji">{item.icon}</span>
            <div>
              <h3>{item.label}</h3>
  ...
  function renderPopup() {
    return <div className="appIconPopupBackdrop" role="dialog" aria-modal="true">
  ...
  const majorTiles = [
    { label: "Events", icon: "📅", href: "/events", text: descriptions.events },
    { label: "Games", icon: "🎮", href: "/home/games", text: descriptions.games },
    { label: "Experiences", icon: "✨", href: "/home/experiences", text: descriptions.experiences },
    { label: "About Us", icon: "🍸", href: "/home/about", text: descriptions.about }
  ];

  if (section === "games" && detail === "trivia") {
    return renderLeaf({
      mark: "GAMES / TRIVIA",
      title: "Trivia",
      description: "Choose the trivia format that fits the room, theme, and crowd.",
      backHref: "/home/games",
      backLabel: "Back to Games",
      items: [
        { label: "General Trivia", icon: "🧠", text: "Classic mixed-category questions for broad crowds and weekly trivia nights." },
        { label: "Themed Trivia", icon: "🎬", text: "A focused theme night built around a movie, decade, holiday, artist, fandom, or sponsor." },
        { label: "Music Trivia", icon: "🎵", text: "Name-that-song, artist, lyrics, eras, and music knowledge rounds." },
        { label: "Survivor Trivia", icon: "🏆", text: "Players or teams stay alive through elimination-style trivia until a final winner remains." }
      ]
    });
  }

  if (section === "games" && detail === "bingo") {
    return renderLeaf({
      mark: "GAMES / BINGO",
      title: "Bingo",
      description: "Pick the bingo format based on how fast, social, or music-driven the event should feel.",
      backHref: "/home/games",
      backLabel: "Back to Games",
      items: [
        { label: "Blackout Bingo", icon: "⬛", text: "A race to cover the full card, built for simple play and clear prize moments." },
        { label: "Quiz Bingo", icon: "❓", text: "Bingo mixed with questions, clues, and answers for a more interactive game night." },
        { label: "Music Bingo", icon: "🎶", text: "Guests listen for songs instead of numbers and mark the matching spaces on their cards." }
      ]
    });
  }

  if (section === "games" && detail === "murder-mystery") {
    return renderLeaf({
      mark: "GAMES / MURDER MYSTERY",
      title: "Murder Mystery",
      description: "A role-based mystery experience where guests use clues, timed reveals, and interaction to solve the case.",
      backHref: "/home/games",
      backLabel: "Back to Games",
      items: [
        { label: "Suspect Roles", icon: "🧑‍💼", text: "Participants can receive roles, alibis, secrets, and social objectives." },
        { label: "Timed Clues", icon: "⏱️", text: "Clues can unlock over time so the game does not get solved too quickly." },
        { label: "Final Accusation", icon: "🕵️", text: "Players submit their solve and compare their deduction against the true answer." }
      ]
    });
  }

  if (section === "games" && detail === "escape-room") {
    return renderLeaf({
      mark: "GAMES / ESCAPE ROOM",
      title: "Escape Room",
      description: "A timed digital puzzle mission designed for bar teams, phones, and live-host energy.",
      backHref: "/home/games",
      backLabel: "Back to Games",
      items: [
        { label: "Timed Mission", icon: "⏳", text: "Guests race against a countdown while the story unfolds." },
        { label: "Puzzle Rounds", icon: "🧩", text: "Each round unlocks a clue, code, cipher, or decision needed to escape." },
        { label: "Hint Control", icon: "💡", text: "Hints can be released on a timer to slow gameplay and protect the experience." }
      ]
    });
  }
  ...
  if (section === "games") {
    return renderMenu({
      mark: "GAMES",
      title: "Games",
      description: descriptions.games,
      backHref: "/home",
      backLabel: "Back to Home",
      showHome: false,
      items: [
        { label: "Trivia", icon: "❓", href: "/home/games/trivia", text: "General Trivia • Themed Trivia • Music Trivia • Survivor Trivia" },
        { label: "Bingo", icon: "🎱", href: "/home/games/bingo", text: "Blackout Bingo • Quiz Bingo • Music Bingo" },
        { label: "Murder Mystery", icon: "🕵️", href: "/home/games/murder-mystery", text: "Roles • Clues • Timed reveals • Final accusation" },
        { label: "Escape Room", icon: "🔐", href: "/home/games/escape-room", text: "Digital puzzle missions with countdowns, clues, and codes" }
      ]
    });
  }

  if (section === "experiences" && detail === "golf-simulator") {
    return renderLeaf({
      mark: "EXPERIENCES / GOLF",
      title: "Golf Simulator",
      description: "Golf activations can be simple putting challenges or a larger full-swing simulator experience.",
      backHref: "/home/experiences",
      backLabel: "Back to Experiences",
      items: [
        { label: "Putting", icon: "⛳", text: "A compact putting challenge built for bars, patios, sponsor activations, and tournaments." },
        { label: "Full Swing", icon: "🏌️", text: "A premium full-swing simulator format for larger events and golf-centered experiences." }
      ]
    });
  }
  ...
  if (section === "experiences") {
    return renderMenu({
      ...
      showHome: false,
      items: [
        { label: "Golf Simulator", icon: "⛳", href: "/home/experiences/golf-simulator", text: "Putting • Full Swing" },
        { label: "Meet & Mingle", icon: "🤝", href: "/meet-mingle", text: "Guided social matching, RSVP, check-in, timed rounds, and prompts" },
        { label: "Vote", icon: "🗳️", href: "/vote", text: "Audience voting, contests, polls, and winner selection" },
        { label: "Karaoke", icon: "🎤", href: "/karaoke", text: "Song requests, singers, stage energy, and hosted karaoke nights" },
        { label: "Social Wall", icon: "📸", href: "/social-wall", text: "Live selfies, posts, and shared social moments on screen" },
        { label: "Barfly TV", icon: "📺", href: "/barfly-tv", text: "Stream live events, sponsor content, ads, and entertainment drops" }
      ]
    });
  }

  if (section === "about" && detail === "advertise") {
    const adEmail = emailLink(settings?.advertiseWithUsEmail || settings?.businessEmail || BUSINESS_EMAIL);
    const adDescription = settings?.advertiseWithUsDescription || "Put your business in front of Barfly Social players with event sponsorships, in-app promo placements, Barfly TV ads, social wall features, prize partnerships, and custom game-night activations. Send us your business name, contact info, goals, preferred dates, and budget range to apply.";
    return <div className="screen publicScreen neonPublic homeLauncherPage hierarchyPage advertiseWithUsPage">
      <PublicBrandHeader />
      <HomeNavButtons backHref="/home/about" backLabel="Back to About Us" showHome />
      <div className="homeLauncherHero hierarchyHero">
        <div className="brandMark">ABOUT US / ADVERTISE</div>
        <h1>Advertise With Us</h1>
        <p className="tagline hierarchyDescription">Reach local players, venues, event crowds, and Barfly Social audiences.</p>
      </div>
      <div className="card neonCard advertiseWithUsCard">
        <span className="appIconEmoji">📣</span>
        <h2>Advertising Opportunities</h2>
        <p>{adDescription}</p>
        <div className="advertiseOpportunityGrid">
          <span>Event sponsorships</span>
          <span>In-app promo placements</span>
          <span>Barfly TV ads</span>
          <span>Social wall features</span>
          <span>Prize partnerships</span>
          <span>Custom activations</span>
        </div>
        {adEmail && <div className="ctaRow"><Button onClick={() => window.location.href = adEmail}>Email to Apply</Button></div>}
      </div>
    </div>;
  }
  ...
  if (section === "about") {
    return renderMenu({
      ...
      showHome: false,
      items: [
        { label: "Jobs", icon: "💼", href: jobsHref || "/jobs", text: jobsHref ? "Open current opportunities." : "Job opportunities will be posted here." },
        { label: "Instagram", icon: "📱", href: instagramHref, text: "Follow Barfly Social and see event updates." },
        { label: "Advertise With Us", icon: "📣", href: "/home/about/advertise", text: "Sponsorships, ads, Barfly TV, social wall, prize partnerships, and custom activations." },
        { label: "Contact", icon: "✉️", href: contactHref, text: "Email Barfly Social for booking and business questions." }
      ].filter(item => item.href)
    });
  }

  return <div className="screen publicScreen neonPublic homeLauncherPage homeMajorOnlyPage">
    <PublicBrandHeader />
    ...
    <div className="appIconGrid homeMajorIconGrid">
      {majorTiles.map(tile => <button type="button" key={tile.label} className="appIconTile homeActionTile majorHomeTile" onClick={() => openTile(tile)}>
...
function SimpleFallbackPage({ type }) {
  const configs = {
    karaoke: {
      mark: "KARAOKE",
      title: "Check the calendar for upcoming karaoke events.",
      body: "No karaoke link is active right now.",
      primary: "View Events Calendar",
      href: "/events"
    },
    mystery: {
      mark: "MYSTERY",
      title: "We are uploading another mystery soon. Please check back later.",
      body: "A new mystery experience is coming soon.",
      primary: "Back to Home",
      href: "/home"
    },
    "escape-room": {
      mark: "ESCAPE ROOM",
      title: "We are uploading another escape room soon. Please check back later.",
      body: "A new escape room experience is coming soon.",
      primary: "Back to Home",
      href: "/home"
    },
    "golf-simulator": {
      mark: "GOLF SIMULATOR",
      title: "Putting simulators are available. Full swing simulators coming soon.",
      body: "Check the calendar for upcoming golf simulator events.",
      primary: "View Events Calendar",
      href: "/events?category=golf"
    },
    "meet-mingle": {
      mark: "MEET & MINGLE",
      title: "Meet & Mingle is a guided social mixer.",
      body: "Guests RSVP, check in, meet through timed rounds, answer prompts, and make real connections.",
      primary: "View Meet & Mingle Events",
      href: "/events?category=meet-mingle"
    },
    jobs: {
      mark: "JOBS",
      title: "We are uploading jobs soon. Please check back later.",
      body: "Job opportunities will be posted here when available.",
      primary: "Back to Home",
      href: "/home"
    }
  };
  const config = configs[type] || configs.mystery;
  return <div className="screen publicScreen neonPublic fallbackMessagePage">
    <div className="card neonCard fallbackMessageBox">
      <div className="brandMark">{config.mark}</div>
      <h1>{config.title}</h1>
      <p className="muted">{config.body}</p>
      <div className="ctaRow">
        <Button onClick={() => window.location.href = config.href}>{config.primary}</Button>
      </div>
    </div>
    <BackToHomeButton />
  </div>;
...
function KaraokePage() {
  const settings = usePublicSettings();
  const karaokeUrl = normalizeExternalUrl(settings?.karaokeUrl || "");

  React.useEffect(() => {
    if (karaokeUrl) {
      window.location.href = karaokeUrl;
    }
  }, [karaokeUrl]);

  if (karaokeUrl) return <div className="screen publicScreen neonPublic fallbackMessagePage">
    <div className="card neonCard fallbackMessageBox">
      <h2>Opening Karaoke...</h2>
      <p className="muted">If it does not open automatically, use the button below.</p>
      <Button onClick={() => window.location.href = karaokeUrl}>Open Karaoke</Button>
    </div>
    <BackToHomeButton />
  </div>;

  return <SimpleFallbackPage type="karaoke" />;
}

function JobsPage() {
  const settings = usePublicSettings();
  const jobsUrl = normalizeExternalUrl(settings?.jobsUrl || "");
  ...
  if (jobsUrl) return <div className="screen publicScreen neonPublic fallbackMessagePage">
    <div className="card neonCard fallbackMessageBox">
      <h2>Opening Jobs...</h2>
      <p className="muted">If it does not open automatically, use the button below.</p>
      <Button onClick={() => window.location.href = jobsUrl}>Open Jobs</Button>
    </div>
    <BackToHomeButton />
  </div>;

  return <SimpleFallbackPage type="jobs" />;
}

function MysteryPage() {
  const settings = usePublicSettings();
  const mysteryUrl = normalizeExternalUrl(settings?.globalMysteryIframeUrl || "");
  if (mysteryUrl) {
    window.location.replace("/game/mystery");
    return null;
  }
  return <SimpleFallbackPage type="mystery" />;
}

function EscapeRoomPage() {
  const settings = usePublicSettings();
  const escapeRoomUrl = normalizeExternalUrl(settings?.escapeRoomUrl || "");
  if (escapeRoomUrl) {
    window.location.replace("/game/escape-room");
    return null;
  }
  return <SimpleFallbackPage type="escape-room" />;
}


function GolfSimulatorPage() {
  const settings = usePublicSettings();
  const golfSimulatorUrl = normalizeExternalUrl(settings?.golfSimulatorUrl || "");
  if (golfSimulatorUrl) {
    window.location.replace("/game/golf-simulator");
    return null;
  }
  return <SimpleFallbackPage type="golf-simulator" />;
}



function MeetMinglePage() {
  const settings = usePublicSettings();
  const meetMingleUrl = normalizeExternalUrl(settings?.meetMingleUrl || "");
  if (meetMingleUrl) {
    window.location.replace("/game/meet-mingle");
    return null;
  }
  return <SimpleFallbackPage type="meet-mingle" />;
}
...
function GameIframePage() {
  ...
  const settings = usePublicSettings();
  const tool = globalIframeTools(settings).find(item => item.key === gameKey);

  if (!tool && gameKey === "mystery") return <SimpleFallbackPage type="mystery" />;
  if (!tool && gameKey === "escape-room") return <SimpleFallbackPage type="escape-room" />;
  if (!tool && gameKey === "golf-simulator") return <SimpleFallbackPage type="golf-simulator" />;
  if (!tool && gameKey === "meet-mingle") return <SimpleFallbackPage type="meet-mingle" />;
...
function BusinessDemo() {
  ...
  const eventTypes = [
    "Arcade & Retro Game Nights",
    "Bachelor / Bachelorette Parties",
    "Bar & Restaurant Game Nights",
    "Barfly TV Live Recaps",
    "Bingo Nights",
    "Birthday Parties",
  ...
  const comparisonRows = [
    {
      feature: "Games per night",
      barfly: "Multiple experiences in one night — bingo, trivia, karaoke, mystery, escape room, Meet & Mingle, voting, and more.",
      traditional: "Often limited to one paper-based game or one host format per night."
    },
    {
      feature: "Gameplay format",
      barfly: "Digital, phone-friendly, and easy to access with event pages and QR links.",
      traditional: "Usually paper-only games, printed sheets, or manual sign-up."
    },
    {
      feature: "Guest engagement tools",
      barfly: "Includes Meet & Mingle, voting, social wall, Barfly TV, and sponsor moments.",
      traditional: "Typically limited to calling the game and basic crowd interaction."
    },
    {
      feature: "Promotional tools",
      barfly: "Supports event pages, sponsor placements, and YouTube Shorts, TikTok, and Instagram promo links.",
      traditional: "Often relies on flyers or social posts outside the event flow."
    },
    {
      feature: "Sponsor support",
      barfly: "Attach sponsors to events, show offers, and highlight prizes inside the app.",
      traditional: "Usually manual shout-outs with limited tracking or digital visibility."
    },
    {
      feature: "Event flexibility",
      barfly: "Works for public events, private parties, activations, and custom business experiences.",
      traditional: "Commonly focused on a single recurring format with fewer custom options."
    },
    {
      feature: "Custom branding",
      barfly: "Venue-friendly pages, event-specific content, and a polished branded experience.",
      traditional: "Usually generic hosting with fewer built-in branded touchpoints."
    },
    {
      feature: "Social + interactive experiences",
      barfly: "Offers more than trivia, bingo, or karaoke alone by combining games with social experiences.",
      traditional: "Usually entertainment only, without added social or digital layers."
    },
    {
      feature: "One platform",
      barfly: "Events, games, experiences, sponsors, and promo links live in one app.",
      traditional: "Often uses separate tools or manual processes for each part of the night."
    },
    {
      feature: "Pricing guarantee",
      barfly: "If we can’t beat their price, it’s free!",
      traditional: "No Barfly pricing guarantee."
    }
  ];
  ...
  return <div className="screen publicScreen neonPublic businessDemo simplifiedDemoPage">
    ...
    <div className="businessHero simplifiedDemoHero">
      ...
      <p className="heroCopy">Barfly Social helps venues promote events, launch games, run social mixers, collect RSVPs, attach sponsors, and keep guests engaged from one clean app.</p>
      <div className="demoGuaranteeBanner">If we can’t beat their price, it’s free!</div>
    </div>

    <div className="card neonCard simplifiedDemoSection comparisonDemoSection">
      <div className="brandMark">WHY BARFLY</div>
      <h2>Barfly Social vs. traditional paper-only or single-game hosts</h2>
      <p className="muted comparisonIntro">Compare what Barfly Social offers against traditional hosts that rely on paper games or can only offer one type of game per night.</p>
      ...
      <div className="comparisonTable" role="table" aria-label="Barfly Social comparison chart">
        ...
        {comparisonRows.map(row => <React.Fragment key={row.feature}>
          <div className="comparisonCell comparisonFeature" role="rowheader">{row.feature}</div>
          <div className="comparisonCell comparisonBarfly" role="cell">{row.barfly}</div>
          <div className="comparisonCell comparisonTraditional" role="cell">{row.traditional}</div>
        </React.Fragment>)}
      </div>
    </div>
    ...
    <div className="card neonCard simplifiedDemoSection">
      <div className="brandMark">EVENTS WE SUPPORT</div>
...
function HostGameIframeSettingsPanel() {
  ...
  function pickGameSettings(settings = {}) {
    return {
      ...
      escapeRoomUrl: settings.escapeRoomUrl || "",
      golfSimulatorUrl: settings.golfSimulatorUrl || "",
      meetMingleUrl: settings.meetMingleUrl || "",
  ...
  async function saveGameSettings() {
    const cleaned = {
      ...
      escapeRoomUrl: normalizeIframeUrl(form.escapeRoomUrl),
      golfSimulatorUrl: normalizeIframeUrl(form.golfSimulatorUrl),
      meetMingleUrl: normalizeIframeUrl(form.meetMingleUrl),
  ...
  return <div className="card glowCard hostGameIframeSettings cleanIframeSettings" id="host-game-iframes">
    ...
    <div className="twoCol iframeUrlGrid">
      ...
      <div><label>Karaoke URL</label><input value={form.karaokeUrl} onChange={e => update("karaokeUrl", e.target.value)} placeholder="https://..." autoComplete="off" /><p className="microcopy">Opens external Karaoke link. Blank shows calendar message.</p></div>
      <div><label>Escape Room URL</label><input value={form.escapeRoomUrl} onChange={e => update("escapeRoomUrl", e.target.value)} placeholder="https://..." autoComplete="off" /><p className="microcopy">Shows Escape Room iframe. Blank shows coming soon message.</p></div>
      <div><label>Golf Simulator URL</label><input value={form.golfSimulatorUrl} onChange={e => update("golfSimulatorUrl", e.target.value)} placeholder="https://..." autoComplete="off" /><p className="microcopy">Shows Golf Simulator iframe. Blank shows simulator message.</p></div>
      <div><label>Meet & Mingle Game URL</label><input value={form.meetMingleUrl} onChange={e => update("meetMingleUrl", e.target.value)} placeholder="https://..." autoComplete="off" /><p className="microcopy">Shows Meet & Mingle game iframe. Blank shows explanation screen.</p></div>
      <div><label>Jobs URL</label><input value={form.jobsUrl} onChange={e => update("jobsUrl", e.target.value)} placeholder="https://..." autoComplete="off" /><p className="microcopy">Opens external Jobs link. Blank shows coming soon message.</p></div>
...
function HostSettingsPanel() {
  ...
  return <div className="card glowCard hostSettingsPanel" id="host-settings-panel">
    ...
    <div className="twoCol">
      ...
      <div><label>Advertising Email</label><input value={settings.advertiseWithUsEmail || ""} onChange={e => update("advertiseWithUsEmail", e.target.value)} placeholder="ads@barfly.social" /></div>
    </div>

    <div className="promoSettingsBox advertiseWithUsSettingsBox">
      <div className="brandMark">ADVERTISE WITH US</div>
      <h3>About Us Advertising Card</h3>
      <p className="microcopy">This controls the description shown when someone clicks Advertise With Us under About Us.</p>
      <label>Advertising Description</label>
      <textarea value={settings.advertiseWithUsDescription || ""} onChange={e => update("advertiseWithUsDescription", e.target.value)} placeholder="Explain sponsorships, ads, Barfly TV, social wall, prize partnerships, and how to apply." />
    </div>

    <div className="promoSettingsBox homeIconDescriptionSettingsBox">
      <div className="brandMark">HOME SCREEN ICON DESCRIPTIONS</div>
      <h3>Four Main Home Icons</h3>
      <p className="microcopy">These text boxes control the description shown on each major Home icon and the top of each section page.</p>
      <label>Events Icon Description</label>
      <textarea value={settings.homeEventsDescription || ""} onChange={e => update("homeEventsDescription", e.target.value)} placeholder="Explain what Events includes." />
      <label>Games Icon Description</label>
      <textarea value={settings.homeGamesDescription || ""} onChange={e => update("homeGamesDescription", e.target.value)} placeholder="Explain what Games includes." />
      <label>Experiences Icon Description</label>
      <textarea value={settings.homeExperiencesDescription || ""} onChange={e => update("homeExperiencesDescription", e.target.value)} placeholder="Explain what Experiences includes." />
      <label>About Us Icon Description</label>
      <textarea value={settings.homeAboutDescription || ""} onChange={e => update("homeAboutDescription", e.target.value)} placeholder="Explain what About Us includes." />
    </div>
...
function HostEventsPanel() {
  ...
  async function loadVenueAndTemplateOptions() {
    ...
    const sponsorData = await sponsorRes.json().catch(() => ({ sponsors: [] }));
  ...
  function editEvent(event) {
    ...
    setForm({
      ...
      sponsorIds: Array.isArray(event.sponsorIds) ? event.sponsorIds : [],
  ...
  function toggleEventSponsor(sponsorId) {
    setForm(current => {
      const currentIds = Array.isArray(current.sponsorIds) ? current.sponsorIds : [];
      const exists = currentIds.includes(sponsorId);
      return {
        ...current,
        sponsorIds: exists ? currentIds.filter(id => id !== sponsorId) : [...currentIds, sponsorId]
      };
    });
  }

  function selectedSponsorNames(ids = []) {
    const selectedIds = Array.isArray(ids) ? ids : [];
    return sponsors.filter(sponsor => selectedIds.includes(sponsor.id)).map(sponsor => sponsor.name);
  }
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
      ...
      <div className="wizardStep">
        ...
        <div>
          ...
          <div className="twoCol">
            ...
            <div><label>YouTube Short URL</label><input value={form.youtubeShortUrl} onChange={e => update("youtubeShortUrl", e.target.value)} placeholder="https://youtube.com/shorts/..." /></div>
            <div><label>TikTok URL</label><input value={form.tiktokUrl} onChange={e => update("tiktokUrl", e.target.value)} placeholder="https://www.tiktok.com/@.../video/..." /></div>
            <div className="fullWidth"><label>Instagram Reel / Post URL</label><input value={form.instagramPostUrl} onChange={e => update("instagramPostUrl", e.target.value)} placeholder="https://www.instagram.com/reel/... or /p/..." /></div>
          </div>
        </div>
      </div>
    ...
    <details className="advancedEventDetails">
      <summary><b>Advanced Event Details</b><span>Open for logos, buttons, social promo URLs, payments, sponsors, manual hero URLs, and advanced Meet & Mingle settings.</span></summary>
      <div className="twoCol">
      <div><label>Saved Venue</label><select value={form.venueId} onChange={e => applyVenue(e.target.value)}>
    ...
    <div className="card miniSettingsCard sponsorAttachBox">
      <div className="brandMark">ATTACH SPONSORS</div>
      <h3>Saved Sponsor Profiles</h3>
      <p className="microcopy">Attach one or more saved sponsors to this event. The public sponsor block stays hidden when nothing is selected.</p>
      {sponsors.length === 0 && <p className="muted">No saved sponsors yet. Add sponsors from the Sponsors section in the host dashboard.</p>}
      {sponsors.length > 0 && <div className="sponsorAttachGrid">
        {sponsors.map(sponsor => <label key={sponsor.id} className="sponsorAttachOption">
          <input type="checkbox" checked={(form.sponsorIds || []).includes(sponsor.id)} onChange={() => toggleEventSponsor(sponsor.id)} />
          {sponsor.logoUrl && <img src={sponsor.logoUrl} alt={sponsor.name} />}
          <span><b>{sponsor.name}</b><small>{[sponsor.category, sponsor.offerTitle].filter(Boolean).join(" • ")}</small></span>
        </label>)}
      </div>}
      {(form.sponsorIds || []).length > 0 && <p className="microcopy">Attached: {selectedSponsorNames(form.sponsorIds).join(" • ")}</p>}
    </div>
    ...
    <div className="card miniSettingsCard socialPromoUrlBox">
      <div className="brandMark">SOCIAL PROMO URLS</div>
      <h3>Optional public promo buttons</h3>
      <p className="microcopy">Paste links for the event. If a field is blank, that public button will not show.</p>
      <div className="twoCol">
        <div><label>YouTube Short URL</label><input value={form.youtubeShortUrl} onChange={e => update("youtubeShortUrl", e.target.value)} placeholder="https://youtube.com/shorts/..." /></div>
        <div><label>TikTok URL</label><input value={form.tiktokUrl} onChange={e => update("tiktokUrl", e.target.value)} placeholder="https://www.tiktok.com/@.../video/..." /></div>
        <div className="fullWidth"><label>Instagram Reel / Post URL</label><input value={form.instagramPostUrl} onChange={e => update("instagramPostUrl", e.target.value)} placeholder="https://www.instagram.com/reel/... or /p/..." /></div>
    ...
    {events.map(event => <div className="rsvpRow" key={event.occurrenceKey || event.id}>
      <div>
        ...
        <p className="microcopy">Social promo links: {eventSocialLinks(event).map(link => link.label).join(" • ") || "none"}</p>
        <p className="microcopy">Sponsors: {eventSponsorNames(event).join(" • ") || "none"}</p>
...
function HostManager() {
  ...
  const hostTiles = [
    ...
    { key: "sponsors", icon: "🤝", title: "Sponsors", text: "Sponsor profiles, offers, logos, and event attachments." },
  ...
  if (selectedHostSection === "sponsors") return <HostPanelShell title="Sponsors" actionMessage={actionMessage} onBack={() => setSelectedHostSection("")}>
    <HostSponsorManager />
  </HostPanelShell>;
...
function App() {
  ...
  if (parts[0] === "karaoke") return <KaraokePage />;
  if (parts[0] === "mystery") return <MysteryPage />;
  if (parts[0] === "escape-room") return <EscapeRoomPage />;
  if (parts[0] === "golf-simulator") return <GolfSimulatorPage />;
  if (parts[0] === "meet-mingle") return <MeetMinglePage />;
  if (parts[0] === "jobs") return <JobsPage />;
  if (parts[0] === "game" && parts[1] === "karaoke") {
