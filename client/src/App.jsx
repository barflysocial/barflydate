...
const onlineEventTypes = ["bingo", "trivia", "music_bingo", "mystery", "escape_room"];

function isOnlineGameEvent(event) {
  const link = String(event?.buttonLink || "");
  return Boolean(event?.onlineOnly) || onlineEventTypes.includes(event?.eventType) || link.includes("/elpasobingo") || link.includes("/pelicantomars");
}
...
function eventActionBadge(event) {
  ...
  if (isBarflySocialEvent(event)) return "BARFLY SOCIAL";
...
function eventPrimaryAction(event) {
  const isBarflySocial = isBarflySocialEvent(event);
  const isOnline = isOnlineGameEvent(event);
  const hasMeetMingle = event?.meetMingleEnabled !== false && event?.meetMingleGameId;
  const paidLabel = event?.buttonText && event.buttonText !== "More Info" ? event.buttonText : "Buy Ticket";
  const meetMingleLabel = event?.buttonText && event.buttonText !== "More Info" ? event.buttonText : "RSVP";
  return {
    label: eventUnavailable(event)
      ? eventStatusLabel(event.eventStatus)
      : (event?.paidEvent ? paidLabel : (hasMeetMingle ? meetMingleLabel : (event?.buttonText || (isBarflySocial ? "View Meet & Mingle / RSVP" : isOnline ? "Play Now" : "More Info")))),
    link: event?.paidEvent
      ? (event?.paymentLink || event?.buttonLink || event?.publicPath || `/events/${event?.slug || event?.id}`)
      : (hasMeetMingle ? `/rsvp?gameId=${event.meetMingleGameId}` : (event?.buttonLink || (isBarflySocial ? "/forecast" : (event?.publicPath || `/events/${event?.slug || event?.id}`))))
  };
}
...
function openEventPrimaryAction(event) {
  ...
  const action = eventPrimaryAction(event);
  trackEventMetric("primary_click", event, { label: action.label, link: action.link });
  setTimeout(() => { window.location.href = action.link; }, 80);
}
...
const APP_VERSION = "BARFLYDATE v76";
...
function eventCardActionLabel(event) {
  if (event?.meetMingleEnabled !== false && event?.meetMingleGameId) return "RSVP";
  const type = String(event?.eventType || "").toLowerCase();
  const title = String(event?.title || "").toLowerCase();
  if (type === "karaoke" || title.includes("karaoke")) return "Song Request";
  if (type === "barfly_social" || title.includes("meet") || title.includes("mingle")) return "RSVP";
  if (type === "bingo" || type === "trivia" || title.includes("bingo") || title.includes("trivia")) return event?.onlineOnly ? "Play" : "RSVP";
  if (type === "mystery" || type === "escape_room" || title.includes("mystery") || title.includes("escape")) return "Join";
  return event?.paidEvent ? "Buy Ticket" : "Join In";
}
...
function AppJourney({ mode, activeRound = 0 }) {
  ...
  return <div>
    ...
    <p className="microcopy journeyNote">App Journey now matches the Join sparks/interests list for entertainment and social matching.</p>
  </div>;
}
...
function PublicBrandHeader() {
  return <div className="publicBrandHeader">
    <button type="button" className="publicBrandLogoButton" onClick={() => window.location.href = "/radar"} aria-label="Barfly Social home">
...
function SplashScreen() {
  ...
  const HOME_TUTORIAL_KEY = "barfly_home_tutorial_dismissed_v1";

  const [ready, setReady] = React.useState(false);
  const [stage, setStage] = React.useState("splash");
  const [deviceType, setDeviceType] = React.useState("iphone");
  ...
  function neverShowAgain() {
    localStorage.setItem(HOME_TUTORIAL_KEY, "1");
    continueToApp();
  }
  ...
  React.useEffect(() => {
    ...
    fetch(`${API}/forecast`)
      ...
      .finally(() => {
        ...
        setTimeout(() => {
          if (cancelled) return;
          const dismissed = localStorage.getItem(HOME_TUTORIAL_KEY) === "1";
          setStage(dismissed ? "redirect" : "prompt");
        }, remaining);
      });
  ...
  React.useEffect(() => {
    if (!ready) return;
    const t = setTimeout(() => { window.location.href = "/radar"; }, SPLASH_FADE_MS);
  ...
  const instructions = isIphone ? [
    ...
    "Tap Add to place Barfly Social on your home screen."
  ] : [
    "Tap the 3-dot menu in Chrome.",
    "Choose Add to Home screen or Install app.",
    "Confirm the app name if asked.",
    "Tap Add to place Barfly Social on your home screen."
  ];
  ...
  return <div className={`screen splashScreen ${ready ? "fadeOut" : ""}`}>
    ...
    <div className="splashCenter">
      ...
      {stage === "prompt" && <div className="homeTipCard">
        <div className="brandMark">QUICK TIP</div>
        <h2>Save Barfly Social to your home screen</h2>
        <p className="muted">Get back to events faster and make Barfly Social feel like an app on your phone.</p>
        <div className="ctaRow">
          <Button onClick={() => setStage("tutorial")}>Show Me</Button>
          <Button className="secondary" onClick={continueToApp}>Maybe Later</Button>
          <Button className="secondary" onClick={neverShowAgain}>Don’t Show Again</Button>
        </div>
      </div>}

      {stage === "tutorial" && <div className="homeTipCard tutorialCard">
        <div className="brandMark">SAVE TO HOME SCREEN</div>
        <h2>{isIphone ? "iPhone Instructions" : "Android Instructions"}</h2>
        <div className="tutorialAnimation">
          <div className="phoneMock">
            <div className="phoneNotch" />
            <div className="phoneScreen">
              <div className="miniHeader">app.barfly.social</div>
              <div className="miniPage">
                <div className="miniLogo">🪰</div>
                <div className="miniLine long" />
                <div className="miniLine" />
                <div className="miniLine short" />
                <div className={isIphone ? "highlightBubble sharePulse" : "highlightBubble menuPulse"}>
                  {isIphone ? "Share" : "Menu"}
                </div>
              </div>
            </div>
          ...
          <div className="tutorialSteps">
            {instructions.map((step, index) => <div key={step} className="tutorialStep">
              <span>{index + 1}</span>
              <p>{step}</p>
            </div>)}
          </div>
        ...
        <div className="ctaRow">
          <Button onClick={continueToApp}>Continue to Meet & Mingle</Button>
          <Button className="secondary" onClick={neverShowAgain}>Don’t Show Again</Button>
        </div>
      </div>}
    </div>
    ...
    {Object.entries(grouped).map(([day, dayEvents]) => <details key={day} className="eventDayBlock collapsibleDay" open={true}>
      ...
      <div className="eventCardGrid">
        {dayEvents.map(event => {
          ...
          return <div className={event.featured ? "card neonCard eventCard premiumEventCard featuredEvent" : "card neonCard eventCard premiumEventCard"} key={event.id}>
            ...
            <div className={`eventTemplateHero template-${template?.slug || "game-night"}`} style={eventHeroStyle(event, template)} aria-label={`${typeLabel} event art`}>
            ...
            <div className="premiumEventInfo">
              ...
              <div className="ctaRow premiumActionRow">
                <Button disabled={eventUnavailable(event)} onClick={() => openEventPrimaryAction(event)}>{eventCardActionLabel(event)}</Button>
...
function EventDetail() {
  ...
  const links = event ? calendarLinksForEvent(event) : null;
  ...
  if (!event || !links) return <div className="screen publicScreen neonPublic"><p>Loading event...</p></div>;
  ...
  return <div className="screen publicScreen neonPublic eventDetailPage">
    ...
    <div className="card neonCard">
      {(event.heroImageUrl || event.eventImageUrl) && <img className="eventDetailImage" src={event.heroImageUrl || event.eventImageUrl} alt={event.title} />}
      ...
      <div className="ctaRow">
        ...
        {!isOnlineGameEvent(event) && !isBarflySocialEvent(event) && !eventUnavailable(event) && <Button className="secondary" onClick={() => respond("going")}>I’m Going</Button>}
        {!isOnlineGameEvent(event) && !isBarflySocialEvent(event) && !eventUnavailable(event) && <Button className="secondary" onClick={() => respond("interested")}>I’m Interested</Button>}
        <Button className="secondary" onClick={() => window.location.href = links.ics}>Add to Calendar</Button>
        <Button className="secondary" onClick={() => setShowCalendarOptions(value => !value)}>More Calendar Options</Button>
        <EventShareButton event={event} />
        <Button className="secondary" onClick={() => copyEventCaption(event, settings)}>Copy Caption</Button>
        <Button className="secondary" onClick={() => window.location.href = `/poster/${event.slug || event.id}`}>Poster</Button>
      </div>
      {showCalendarOptions && <div className="calendarOptions">
        <Button className="secondary" onClick={() => window.location.href = links.google}>Google Calendar</Button>
        <Button className="secondary" onClick={() => window.location.href = links.ics}>Apple / Outlook (.ics)</Button>
      </div>}
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    ...
    <div className="eventCardGrid">
      {tonight.map(event => <div className="card neonCard eventCard featuredEvent" key={event.id}>
        ...
        <div className="ctaRow"><Button onClick={() => openEventPrimaryAction(event)}>{eventPrimaryAction(event).label}</Button><Button className="secondary" onClick={() => window.location.href = event.publicPath}>Event Page</Button><EventShareButton event={event} /></div>
      </div>)}
    </div>
    <PublicNav active="events" />
  </div>;
...
function HostSection({ id, title, subtitle, children }) {
  return <details id={id} className="hostSection">
    <summary>
      <div>
...
function HostQuickActions() {
  ...
  return <div className="card glowCard hostQuickActions" id="host-quick-actions">
    <div className="brandMark">HOST QUICK ACTIONS</div>
    <h2>Command Center</h2>
    <p className="microcopy">Buttons open the correct collapsed section first, then scroll to the tool.</p>
    <div className="quickActionGrid">
      <Button onClick={() => openSection("host-section-marketing", "host-hero-templates")}>Hero Templates</Button>
      <Button className="secondary" onClick={() => openSection("host-section-marketing", "host-poster-builder")}>Poster Builder</Button>
      <Button className="secondary" onClick={() => openSection("host-section-events", "host-events")}>Add Event</Button>
      <Button className="secondary" onClick={() => openSection("host-section-events", "host-event-calendar")}>Event Calendar</Button>
      <Button className="secondary" onClick={() => openSection("host-section-business", "host-analytics")}>View Analytics</Button>
      <Button className="secondary" onClick={() => openSection("host-section-gameplay", "create-social-session")}>Quick Quick Create Meet & Mingle Session</Button>
      <Button className="secondary" onClick={() => openSection("host-section-marketing", "host-qr-tools")}>Generate QR</Button>
      <Button className="secondary" onClick={() => openSection("host-section-business", "host-backup-tools")}>Export Backup</Button>
      <Button className="secondary" onClick={() => openSection("host-section-settings", "host-settings-panel")}>Promo Popup Settings</Button>
    </div>
  </div>;
}
...
function HostEventCalendar() {
  ...
  return <div className="card glowCard hostCalendarPanel" id="host-event-calendar">
    ...
    {selectedEvents.map(event => <div className="rsvpRow hostCalendarRow" key={event.id}>
      <div>
        <div className="compact">
          <b>{event.title}</b>
...
function BusinessDemo() {
  ...
  return <div className="screen publicScreen neonPublic businessDemo">
    ...
    <div id="business-contact" className="card contactCard demoBottomActions">
      <div className="brandMark">NEXT STEP</div>
      <h1>Ready to see what is happening?</h1>
      <p className="tagline">View public events or contact Barfly Social directly.</p>
      <div className="ctaRow demoPrimaryActions">
        <Button onClick={() => setShowContactOptions(value => !value)}>Contact</Button>
        <Button onClick={() => window.location.href = "/events"}>View Events Calendar</Button>
      </div>
      {showContactOptions && <div className="contactOptionsCard">
        {demoPhone && <Button className="secondary" onClick={() => window.location.href = demoPhone}>Call</Button>}
        {demoEmail && <Button className="secondary" onClick={() => window.location.href = demoEmail}>Email</Button>}
        {!demoPhone && !demoEmail && <p className="muted">Add a business phone or email in Host Settings.</p>}
      </div>}
    </div>
  </div>;
}
...
function MyRSVP() {
  ...
  return <div className="screen publicScreen neonPublic">
    ...
    {rsvp && <div className="card neonCard">
      ...
      <div className="ctaRow">
        <Button onClick={() => window.location.href = "/checkin"}>Venue Check-In</Button>
...
function HostHeroTemplateManager() {
  ...
  async function deleteHeroTemplate(template) {
    if (!confirm(`Delete ${template.venueName} ${heroVariantLabel(template.heroVariant)} hero?`)) return;
    ...
    <div className="heroTemplateList">
      {heroTemplates.map(template => <div className="rsvpRow heroTemplateRow" key={template.id}>
        <div>
          ...
          <p className="muted">{template.active === false ? "Inactive" : "Active"} • {template.heroImageUrl || "No image URL yet"}</p>
        ...
        <div className="row">
          ...
          <Button className="danger" onClick={() => deleteHeroTemplate(template)}>Delete</Button>
        </div>
      </div>)}
    </div>
  </div>;
...
function HostEventsPanel() {
  const blank = {
    ...
    meetMingleEnabled: true,
    meetMingleGameId: "",
    meetMingleGameMode: "full_90",
    meetMingleMaxRsvps: 60,
    meetMingleMaxCheckins: 60,
    meetMinglePointMode: "single",
    meetMingleZones: "Host Stand",
    meetMingleDrinkSpecialTitle: "",
    meetMingleDrinkSpecialDetails: "",
    meetMingleDrinkSpecialWindow: "During BARFLYDATE session only",
    meetMingleDrinkSpecialRestrictions: "21+, venue rules apply",
    title: "Workday Blackout Bingo",
    eventType: "bingo",
    customEventType: "",
  ...
  async function loadVenueAndTemplateOptions() {
    const [venueRes, templateRes, heroRes] = await Promise.all([
      hostFetch(`${API}/host/venues`),
      hostFetch(`${API}/host/event-templates`),
      hostFetch(`${API}/host/hero-templates`)
    ]);
    const venueData = await venueRes.json().catch(() => ({ venues: [] }));
    const templateData = await templateRes.json().catch(() => ({ templates: [] }));
  ...
  function editEvent(event) {
    ...
    setForm({
      ...
      meetMingleEnabled: event.meetMingleEnabled !== false,
  ...
  async function saveEvent() {
    ...
    if (data.meetMingleSession) {
      alert("Event saved and Meet & Mingle session linked.");
    }
    setEditingId("");
    setForm(blank);
    setEvents(data.events || []);
  ...
  async function duplicateEventNextWeek(id) {
    const res = await hostFetch(`${API}/host/events/${id}/duplicate-next-week`, { method: "POST" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.error) return alert(data.error || "Could not duplicate to next week.");
    setEvents(data.events || []);
  }
  ...
  function matchingHeroUrl(venueName, heroVariant) {
    ...
    const match = heroTemplates.find(template =>
  ...
  function applyVenue(venueId) {
    ...
    setForm(current => {
      ...
      const heroUrl = matchingHeroUrl(next.venueName, next.heroVariant);
      return heroUrl ? { ...next, heroImageUrl: heroUrl } : next;
    });
  }
  ...
  function applyTemplate(templateId) {
    ...
    setForm(current => {
      if (!template) return { ...current, templateId };
      const templatePoster = posterTemplateFor(template);
      const next = {
        ...current,
        templateId,
        posterTemplateSlug: current.posterTemplateSlug || templatePoster?.slug || "game-night",
        posterOverlayLayout: current.posterOverlayLayout || templatePoster?.overlayLayout || "bottom_info_panel",
        heroVariant: current.heroVariant || defaultHeroVariantForEvent(template.eventType, template.title || template.customEventType),
        title: template.title || current.title,
        eventType: template.eventType || current.eventType,
        customEventType: template.customEventType || current.customEventType || "",
  ...
  const suggestedHeroUrl = matchingHeroUrl(form.venueName, form.heroVariant);
  ...
  function useSuggestedHero() {
    ...
    setForm(current => ({ ...current, heroImageUrl: suggestedHeroUrl }));
  }
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
    ...
    <div className="card quickEventBuilder">
      ...
      <div className="wizardStep">
        ...
        <div>
          ...
          <div className="heroSuggestion">
            {suggestedHeroUrl ? <><b>Recommended hero found</b><p>{form.venueName} + {heroVariantLabel(form.heroVariant)}</p><Button className="secondary" onClick={useSuggestedHero}>Use This Hero</Button></> : <><b>No saved hero match yet</b><p>Create one in Marketing → Venue Hero Template Manager, or paste a Hero Image URL in Advanced.</p></>}
          </div>
        </div>
      </div>
      ...
      <div className="quickPreviewCard">
        <div>
          ...
          <h3>{form.title}</h3>
          <p>{form.venueName} • {weeklyDayOptions.find(day => day.value === Number(form.dayOfWeek))?.label || "Date"} • {formatEventTime(form.startTime)}–{formatEventTime(form.endTime)}</p>
    ...
    <details className="advancedEventDetails">
      ...
      <div className="twoCol">
      <div><label>Use Template</label><select value={form.templateId} onChange={e => applyTemplate(e.target.value)}>
      ...
      <div><label>Poster Template</label><select value={form.posterTemplateSlug} onChange={e => {
        const selected = posterTemplates.find(template => template.slug === e.target.value);
        setForm(current => ({ ...current, posterTemplateSlug: e.target.value, posterOverlayLayout: selected?.overlayLayout || current.posterOverlayLayout || "bottom_info_panel" }));
      }}>
        {posterTemplates.map(template => <option key={template.slug} value={template.slug}>{template.pickerLabel}</option>)}
      </select><p className="microcopy">Used by the Instagram poster renderer. Event details are overlaid by the system.</p></div>
      <div><label>Overlay Layout</label><select value={form.posterOverlayLayout} onChange={e => update("posterOverlayLayout", e.target.value)}>
        <option value="bottom_info_panel">Bottom Info Panel</option>
        <option value="center_stack">Center Stack</option>
        <option value="split_panel">Split Panel</option>
      </select></div>
      <div><label>Hero Variant</label><select value={form.heroVariant} onChange={e => update("heroVariant", e.target.value)}>
        {heroVariants.map(variant => <option key={variant.value} value={variant.value}>{variant.label}</option>)}
      </select><p className="microcopy">Used to find the correct venue-branded event hero.</p></div>
      <div><label>Hero Image URL</label><input value={form.heroImageUrl} onChange={e => update("heroImageUrl", e.target.value)} placeholder="Auto-filled from Venue + Hero Variant" /><Button className="secondary miniButton" onClick={applyMatchingHero}>Use Matching Venue Hero</Button></div>
      <div><label>Event Title</label><input value={form.title} onChange={e => update("title", e.target.value)} /></div>
      <div><label>Event Category</label><select value={form.eventType} onChange={e => update("eventType", e.target.value)}>{calendarEventTypes.map(type => <option key={type.value} value={type.value}>{type.label}</option>)}</select></div>
    ...
    <div className="twoCol">
      ...
      <div><label>Venue Logo URL</label><input value={form.venueLogoUrl} onChange={e => update("venueLogoUrl", e.target.value)} placeholder="https://..." /><p className="microcopy imageHelp">Venue Logo = where the event is happening.</p></div>
    ...
    {events.map(event => <div className="rsvpRow" key={event.id}>
      <div>
        ...
        <p className="microcopy">Poster Template: {posterTemplateFor(event)?.pickerLabel || "🎮 Game Night"}</p>
...
function HostManager() {
  ...
  const [venueName, setVenueName] = React.useState("Barfly Social Meet & Mingle");
  ...
  return <div className="screen hostManagerCompact">
    ...
    <HostSection id="host-section-gameplay" title="Gameplay" subtitle="Create sessions, test games, active games, and run-of-show.">
      <div className="card checklistCard">
        ...
        <p className="checkOk">3. Watch RSVPs and Meet & Mingle demand.</p>
      ...
      <h2>Quick Create Meet & Mingle Session</h2>
      ...
      <div className="ctaRow"><Button onClick={createGame}>Create Meet & Mingle Session</Button><Button className="secondary" onClick={duplicateLastSession}>Duplicate Last Session</Button></div>
    </div>
    ...
    <HostSection id="host-section-events" title="Events & Calendar" subtitle="Event calendar, booking calendar, booking list, and event editor.">
      <HostEventCalendar />
      <HostBookingCalendar />
      <HostBookingsPanel />
      <HostEventsPanel />
    ...
    <HostSection id="host-section-marketing" title="Marketing" subtitle="Poster Builder, venue hero templates, event templates, QR tools, and promo assets.">
      ...
      <div className="card glowCard" id="host-qr-tools">
        ...
        <div className="ctaRow">
          ...
          <Button className="secondary" onClick={() => window.location.href = "/qr/radar"}>Meet & Mingle QR</Button>
          <Button className="secondary" onClick={() => window.location.href = "/qr/checkin"}>Check-In QR</Button>
        </div>
      </div>
    </HostSection>

    <HostSection id="host-section-business" title="Business" subtitle="Venue partners, analytics, and backup/export tools.">
      <HostVenueManager />
    ...
    <HostSection id="host-section-settings" title="Settings" subtitle="Host and public app settings.">
      <HostSettingsPanel />
    </HostSection>
  </div>;
}
...
function App() {
  ...
  if (parts[0] === "poster" && parts[1]) return <PosterGenerator />;
