...
function eventIframeTools(event, { includeSocialWall = true, gameOnly = false } = {}) {
  if (!event) return [];
  const tools = [
    { key: "socialWall", label: "Social Wall", icon: "📸", url: event.socialWallIframeUrl, title: `${event.title || "Event"} Social Wall`, social: true },
    { key: "bingo", label: "Bingo", icon: "🎱", url: event.bingoIframeUrl, title: `${event.title || "Event"} Bingo` },
    { key: "trivia", label: "Trivia", icon: "❓", url: event.triviaIframeUrl, title: `${event.title || "Event"} Trivia` },
    { key: "mystery", label: "Mystery", icon: "🕵️", url: event.mysteryIframeUrl, title: `${event.title || "Event"} Mystery` },
    { key: "vote", label: "Vote", icon: "🗳️", url: event.voteIframeUrl, title: `${event.title || "Event"} Vote` }
  ];
  return tools
    .filter(tool => includeSocialWall || !tool.social)
    .filter(tool => !gameOnly || !tool.social)
...
function EventGameIconBar({ event, gameOnly = true, fixed = false }) {
  const [activeTool, setActiveTool] = React.useState(null);
  const tools = eventIframeTools(event, { includeSocialWall: !gameOnly, gameOnly });
  if (!tools.length) return null;
  return <>
    <div className={fixed ? "eventGameIconBar fixedEventGameIconBar" : "eventGameIconBar"}>
...
function SocialWallButton({ event }) {
  const [activeTool, setActiveTool] = React.useState(null);
  const tool = eventIframeTools(event, { includeSocialWall: true }).find(item => item.key === "socialWall");
  if (!tool) return null;
  return <>
    <Button className="secondary socialWallEventButton" onClick={() => setActiveTool(tool)}>
...
const APP_VERSION = "BARFLYDATE v93";
...
function EventsCalendar() {
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
        ...
        {dayEvents.map(event => {
          ...
          return <div className={`${event.featured ? "card neonCard eventCard premiumEventCard featuredEvent" : "card neonCard eventCard premiumEventCard"} ${event.showHeroGraphic === false ? "noHeroEventCard" : ""}`} key={event.occurrenceKey || event.id}>
            ...
            <div className="premiumEventInfo">
              ...
              <EventGameIconBar event={event} gameOnly />
            </div>
          </div>;
        })}
      </div>
    ...
    {hasSelectedDateRsvpEvent && <EventsPageActionNav />}
  </div>;
}
...
function EventDetail() {
  ...
  return <div className="screen publicScreen neonPublic eventDetailPage">
    ...
    <EventGameIconBar event={event} gameOnly fixed />
...
function HostEventsPanel() {
  ...
  function editEvent(event) {
    ...
    setForm({
      ...
      socialWallIframeUrl: event.socialWallIframeUrl || "",
      bingoIframeUrl: event.bingoIframeUrl || "",
      triviaIframeUrl: event.triviaIframeUrl || "",
      mysteryIframeUrl: event.mysteryIframeUrl || "",
      voteIframeUrl: event.voteIframeUrl || "",
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
    ...
    <div className="card quickEventBuilder">
      ...
      <div className="wizardStep">
        <span>6</span>
        <div>
          <h4>Event Game Links / Iframes</h4>
          <p className="microcopy">Add iframe URLs for this event. Blank fields hide that icon.</p>
          <div className="twoCol">
            <div><label>Social Wall iframe URL</label><input value={form.socialWallIframeUrl} onChange={e => update("socialWallIframeUrl", e.target.value)} placeholder="https://games.barfly.social/BARFLYSOCIAL" /></div>
            <div><label>Bingo iframe URL</label><input value={form.bingoIframeUrl} onChange={e => update("bingoIframeUrl", e.target.value)} placeholder="https://games.barfly.social/bingo" /></div>
            <div><label>Trivia iframe URL</label><input value={form.triviaIframeUrl} onChange={e => update("triviaIframeUrl", e.target.value)} placeholder="https://games.barfly.social/trivia" /></div>
            <div><label>Mystery iframe URL</label><input value={form.mysteryIframeUrl} onChange={e => update("mysteryIframeUrl", e.target.value)} placeholder="https://games.barfly.social/mystery" /></div>
            <div><label>Vote iframe URL</label><input value={form.voteIframeUrl} onChange={e => update("voteIframeUrl", e.target.value)} placeholder="https://games.barfly.social/vote" /></div>
          </div>
        </div>
      </div>
