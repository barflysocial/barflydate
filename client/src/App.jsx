...
const APP_VERSION = "BARFLYDATE v109";
...
function GamesDashboard() {
  ...
  return <div className="screen publicScreen neonPublic gamesDashboardPage gamesLauncherOnlyPage">
    {tools.length ? <div className="gamesLauncherIconGrid">
      {tools.map(tool => <button type="button" key={tool.key} className="appIconTile gameLaunchTile" onClick={() => window.location.href = tool.url} aria-label={tool.label}>
        <span className="appIconEmoji">{tool.icon}</span>
        <b>{tool.label}</b>
        <small>Open {tool.label}</small>
      </button>)}
    </div> : <div className="card neonCard emptyGamesPanel">
      <h2>No game links added yet.</h2>
      <p className="muted">Add Bingo, Trivia, Mystery, or Vote URLs in /host → Games / Iframes.</p>
    </div>}
...
function SocialWall() {
  ...
  return <div className="screen publicScreen neonPublic socialWallPage focusedIframePage">
    <div className="autoFitIframeGrid socialWallAutoFitGrid">
      {frames.map(frame => <div className="autoFitIframeCard socialWallFrameCard" key={frame.url}>
