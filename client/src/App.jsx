...
const APP_VERSION = "BARFLYDATE v19";
...
function Forecast() {
  ...
  React.useEffect(() => {
    fetch(`${API}/forecast`).then(res => res.json()).then(setData).catch(() => setData({ sessions: [] }));
  }, []);

  if (!data) return <div className="screen">Loading forecast...</div>;

  return <div className="screen premiumEntry">
    <div className="brandMark">TONIGHT’S CROWD FORECAST</div>
    <h1>Find the Right Room</h1>
    <p className="tagline">Forecasts are privacy-friendly. They show interest levels, not names or exact low counts.</p>

    <div className="row">
      <Button onClick={() => window.location.href = "/rsvp"}>RSVP Now</Button>
      <Button className="secondary" onClick={() => window.location.href = "/checkin"}>Check In</Button>
      <Button className="secondary" onClick={() => window.location.href = "/join"}>Join Without RSVP</Button>
    </div>

    {(!data.sessions || data.sessions.length === 0) && <div className="card"><p className="muted">No upcoming sessions yet.</p></div>}

    {(data.sessions || []).map(session => <div className="card glowCard" key={session.id}>
      <div className="compact">
        <div>
          <h2>{session.venueName}</h2>
          <p className="muted">{session.modeLabel} • {session.phaseLabel}</p>
          <p className="microcopy">{session.locked ? "Session started — RSVP locked" : "RSVP open"}</p>
        </div>
        <span className={session.locked ? "statusPill storageWarn" : "statusPill storageOk"}>{session.locked ? "locked" : "open"}</span>
      </div>

      <div className="demandGrid">
        {lookingForOptions.map(option => <div className="demandChip" key={option.value}>
          <span>{option.label}</span>
          <b>{session.forecast?.[option.value]?.level || "Building"}</b>
        </div>)}
      </div>

      <h3>App Journey</h3>
      <AppJourney mode={session.gameMode} />

      <div className="row">
        {!session.locked && <Button onClick={() => window.location.href = `/rsvp?gameId=${session.id}`}>RSVP for This Session</Button>}
        <Button className="secondary" onClick={() => window.location.href = "/checkin"}>Check In with Code</Button>
      </div>
    </div>)}
  </div>;
}
...
function RSVP() {
  ...
  React.useEffect(() => {
    fetch(`${API}/forecast`).then(res => res.json()).then(data => {
      setForecast(data);
      if (!form.gameId && data.sessions?.[0]?.id) {
  ...
  const toggleInterest = item => setForm(current => ({
    ...current,
    interests: current.interests.includes(item) ? current.interests.filter(x => x !== item) : [...current.interests, item]
  }));
  ...
  if (savedRsvp) return <div className="screen premiumEntry">
    <div className="brandMark">RSVP LOCKED IN</div>
    <h1>{savedRsvp.nickname}</h1>
    <p className="tagline">You RSVP’d for {savedRsvp.venueName}.</p>
    <div className="card glowCard">
      <p><b>Looking for:</b> {lookingForLabel(savedRsvp.lookingFor)}</p>
      <p><b>Status:</b> {savedRsvp.status}</p>
      <p className="muted">At the venue, enter the session code to check in. RSVP alone does not put you into the live game.</p>
    </div>
    <div className="row">
      <Button onClick={() => window.location.href = "/checkin"}>Check In at Venue</Button>
  ...
  return <div className="screen premiumEntry">
    ...
    <p className="tagline">Choose your bar/session first. You’ll still need the venue session code to check in.</p>

    {error && <div className="alert">{error}</div>}

    <div className="card glowCard">
      <label>Choose Location / Session</label>
      <select value={form.gameId} onChange={e => update("gameId", e.target.value)}>
        <option value="">Choose a session...</option>
        {(forecast?.sessions || []).filter(session => !session.locked).map(session => <option key={session.id} value={session.id}>{session.venueName} — {session.modeLabel}</option>)}
      </select>
      ...
      <p className="microcopy">Alias is locked across active/upcoming sessions tonight.</p>
    ...
    <div className="card agreementCard">
      <h2>RSVP Agreement</h2>
      <p className="prompt">• You can change your RSVP only before your original session starts and before the new session starts.</p>
      <p className="prompt">• Once the original session starts, this alias is locked for the event.</p>
    ...
    <Button className={!acceptedAgreement ? "disabledBtn" : ""} onClick={submitRsvp}>Submit RSVP</Button>
  </div>;
}
  ...
  return <div className="screen">
    ...
    <div className="card">
      ...
      {rsvps.slice(0, 12).map(rsvp => <div className="rsvpRow" key={rsvp.id}>
        <div>
          <b>{rsvp.nickname}</b>
          <p className="muted">{lookingForLabel(rsvp.lookingFor)} • {rsvp.status} {rsvp.checkedIn ? "• checked in" : ""}</p>
        </div>
