...
const APP_VERSION = "BARFLYDATE v20";
...
const publicNavItems = [
  ...
  { key: "checkin", label: "Check In", icon: "📍", href: "/checkin" },
  { key: "join", label: "Private", icon: "🎟️", href: "/join" }
];
...
function Forecast() {
  ...
  return <div className="screen publicScreen neonPublic">
    <div className="publicHero">
      ...
      <h1>Tonight’s Crowd Forecast</h1>
      <p className="tagline">Find the room that matches your intention before you go. Forecasts are privacy-friendly and show demand, not names.</p>
      <div className="ctaRow"><Button onClick={() => window.location.href = "/rsvp"}>RSVP Now</Button><Button className="secondary" onClick={() => window.location.href = "/checkin"}>Venue Check-In</Button></div>
    </div>
    ...
    {(data?.sessions || []).map(session => <div className="card neonCard" key={session.id}>
      <div className="compact"><div><h2>{session.venueName}</h2>{session.venueLocation && <p className="muted">{session.venueLocation}</p>}<p className="microcopy">{session.modeLabel} • {session.locked ? "Session started — RSVP locked" : "RSVP open"}</p></div><span className={session.locked ? "statusPill storageWarn" : "statusPill storageOk"}>{session.locked ? "locked" : "open"}</span></div>
      ...
      <div className="twoColMeta"><div><div className="metaLabel">RSVPs</div><div className="metaValue">{session.rsvpCount || 0}</div></div><div><div className="metaLabel">Checked In</div><div className="metaValue">{session.checkedInCount || 0}</div></div></div>
      <h3>Session Apps</h3><AppJourney mode={session.gameMode} />
      <div className="ctaRow">{!session.locked && <Button onClick={() => window.location.href = `/rsvp?gameId=${session.id}`}>RSVP for This Session</Button>}<Button className="secondary" onClick={() => window.location.href = "/join"}>Private Event Join</Button></div>
    </div>)}
...
function RSVP() {
  ...
  const [error, setError] = React.useState("");
  const [form, setForm] = React.useState({ gameId: params.get("gameId") || "", nickname: randomAliasName(), persona: personas[0], realName: "", contactHandle: "", gender: "man", interestedIn: "women", lookingFor: "serious_dating", preferredMinAge: 25, preferredMaxAge: 45, interests: ["Food & Drink", "Entertainment"], customQ1: "", customQ2: "" });

  React.useEffect(() => { fetch(`${API}/forecast`).then(res => res.json()).then(data => { setForecast(data); if (!params.get("gameId") && data.sessions?.[0]?.id) setForm(current => ({ ...current, gameId: data.sessions[0].id })); }).catch(() => setForecast({ sessions: [] })); }, []);
  const selectedSession = (forecast?.sessions || []).find(session => session.id === form.gameId);
  function update(key, value) { setForm(current => ({ ...current, [key]: value })); }
  function toggleInterest(item) { setForm(current => ({ ...current, interests: current.interests.includes(item) ? current.interests.filter(entry => entry !== item) : [...current.interests, item] })); }
  async function submitRsvp() {
    setError("");
    if (!acceptedAgreement) { setError("Please accept the RSVP agreement before continuing."); return; }
    const res = await fetch(`${API}/rsvps`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(form) });
    const data = await res.json().catch(() => ({ error: "Server returned an unreadable response." }));
    if (!res.ok || data.error) { setError(data.error || "Could not save RSVP."); return; }
    localStorage.setItem("barflydateRsvpId", data.rsvp.id);
    setSavedRsvp(data.rsvp);
  }

  if (savedRsvp) return <div className="screen publicScreen neonPublic"><div className="publicHero"><div className="brandMark">BARFLYDATE RSVP</div><h1>Spot Saved</h1><p className="tagline">You’re booked for {savedRsvp.venueName}.</p></div><div className="card neonCard"><p><b>Venue:</b> {savedRsvp.venueName}</p>{savedRsvp.venueLocation && <p><b>Location:</b> {savedRsvp.venueLocation}</p>}<p><b>Looking For:</b> {savedRsvp.lookingForLabel || lookingForLabel(savedRsvp.lookingFor)}</p><p><b>Age Range:</b> {savedRsvp.preferredMinAge}–{savedRsvp.preferredMaxAge}</p><p className="muted">At the venue, enter the session code to check in. RSVP alone does not put you into the live game.</p><div className="ctaRow"><Button onClick={() => window.location.href = "/checkin"}>Check In at Venue</Button><Button className="secondary" onClick={() => window.location.href = "/forecast"}>Back to Forecast</Button></div></div><PublicNav active="rsvp" /></div>;

  return <div className="screen publicScreen neonPublic"><div className="publicHero"><div className="brandMark">BARFLYDATE RSVP</div><h1>Save Your Spot</h1><p className="tagline">Five quick steps. Choose your session now, then check in at the venue with the live code.</p><div className="stepTracker">{[1,2,3,4,5].map(number => <span key={number} className={number === step ? "stepDot active" : "stepDot"}>{number}</span>)}</div><p className="microcopy">Step {step} of 5</p></div>
    {error && <div className="alert">{error}</div>}
    <div className="card neonCard">
      {step === 1 && <><h2>1. Choose Location</h2><label>Venue / Session</label><select value={form.gameId} onChange={e => update("gameId", e.target.value)}><option value="">Choose a session...</option>{(forecast?.sessions || []).filter(session => !session.locked).map(session => <option key={session.id} value={session.id}>{session.venueName} — {session.modeLabel}</option>)}</select>{selectedSession && <div className="miniPanel"><b>{selectedSession.venueName}</b>{selectedSession.venueLocation && <p className="muted">{selectedSession.venueLocation}</p>}<p className="microcopy">Forecast-first public flow.</p></div>}<label>Mystery Alias</label><div className="inlineInput"><input value={form.nickname} onChange={e => update("nickname", e.target.value)} /><button type="button" className="miniBtn" onClick={() => update("nickname", randomAliasName())}>Shuffle</button></div></>}
      {step === 2 && <><h2>2. Intent + Age Range</h2><label>Gender</label><select value={form.gender} onChange={e => update("gender", e.target.value)}><option value="man">Man</option><option value="woman">Woman</option><option value="custom">Custom / Prefer not to say</option></select><label>Interested In</label><select value={form.interestedIn} onChange={e => update("interestedIn", e.target.value)}><option value="women">Women</option><option value="men">Men</option><option value="everyone">Everyone</option></select><label>Looking For</label><select value={form.lookingFor} onChange={e => update("lookingFor", e.target.value)}>{lookingForOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><div className="twoCol"><div><label>Preferred Age Min</label><input type="number" min="18" max="99" value={form.preferredMinAge} onChange={e => update("preferredMinAge", e.target.value)} /></div><div><label>Preferred Age Max</label><input type="number" min="18" max="99" value={form.preferredMaxAge} onChange={e => update("preferredMaxAge", e.target.value)} /></div></div></>}
      {step === 3 && <><h2>3. Pick Your Sparks</h2><p className="microcopy">Choose the topics that matter most to you.</p><div className="chips">{categories.map(category => <button type="button" key={category} className={form.interests.includes(category) ? "chip on" : "chip"} onClick={() => toggleInterest(category)}>{emojiFor(category)} {category}</button>)}</div></>}
      {step === 4 && <><h2>4. Custom Questions</h2><label>Your Custom Question #1</label><input placeholder="What kind of energy are you hoping to meet tonight?" value={form.customQ1} onChange={e => update("customQ1", e.target.value)} /><label>Your Custom Question #2</label><input placeholder="What makes someone easy to talk to?" value={form.customQ2} onChange={e => update("customQ2", e.target.value)} /><label>Real Name <span className="muted">optional, mutual only</span></label><input placeholder="Optional" value={form.realName} onChange={e => update("realName", e.target.value)} /><label>Contact Handle <span className="muted">optional, mutual only</span></label><input placeholder="Phone, Instagram, or email" value={form.contactHandle} onChange={e => update("contactHandle", e.target.value)} /></>}
      {step === 5 && <><h2>5. Review + Agree</h2><div className="miniPanel"><p><b>Venue:</b> {selectedSession?.venueName || "Choose a session"}</p>{selectedSession?.venueLocation && <p><b>Location:</b> {selectedSession.venueLocation}</p>}<p><b>Intent:</b> {lookingForLabel(form.lookingFor)}</p><p><b>Age Range:</b> {form.preferredMinAge}–{form.preferredMaxAge}</p><p><b>Sparks:</b> {form.interests.join(", ")}</p></div><div className="agreementCard"><p className="prompt">• You can change your RSVP only before your original session starts and before the new session starts.</p><p className="prompt">• Once the original session starts, this alias is locked for the event.</p><p className="prompt">• You still need the session code at the venue to check in.</p><label className="checkRow"><input type="checkbox" checked={acceptedAgreement} onChange={e => setAcceptedAgreement(e.target.checked)} /><span>I understand and agree.</span></label></div></>}
      <div className="ctaRow">{step > 1 && <Button className="secondary" onClick={() => setStep(step - 1)}>Back</Button>}{step < 5 && <Button onClick={() => setStep(step + 1)}>Continue</Button>}{step === 5 && <Button className={!acceptedAgreement ? "disabledBtn" : ""} onClick={submitRsvp}>Submit RSVP</Button>}</div>
    </div><PublicNav active="rsvp" /></div>;
}
...
function CheckIn() {
  ...
  return <div className="screen premiumEntry">
    ...
    <div className="row">
      ...
      <Button className="secondary" onClick={() => window.location.href = "/join"}>Join Without RSVP</Button>
    </div>
  </div>;
}
...
function HostManager() {
  ...
  async function createGame() {
    const res = await fetch(`${API}/games`, {
      ...
      body: JSON.stringify({ venueName, venueLocation, gameMode })
    });
    const data = await res.json();
    ...
    {games.map(game => <div className="card" key={game.id}>
      <div className="compact">
        <div>
          ...
          <p className="muted">Code: <b>{game.gameCode}</b> • {game.playerCount} checked in • {game.rsvpCount || 0} RSVPs</p>
...
function HostGame() {
  ...
  return <div className="screen">
    ...
    <div className="card">
      ...
      {rsvps.slice(0, 12).map(rsvp => <div className="rsvpRow" key={rsvp.id}>
        <div>
          <b>{rsvp.nickname}</b>
          <p className="muted">{lookingForLabel(rsvp.lookingFor)} • Ages {rsvp.preferredMinAge || 25}-{rsvp.preferredMaxAge || 45} • {rsvp.status} {rsvp.checkedIn ? "• checked in" : ""}</p>
        </div>
        <span className={rsvp.checkedIn ? "statusPill storageOk" : "statusPill"}>{rsvp.checkedIn ? "checked in" : "RSVP"}</span>
      </div>)}
    </div>
