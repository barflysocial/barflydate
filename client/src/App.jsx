...
const APP_VERSION = "BARFLYDATE v80";
    ...
    {Object.entries(grouped).map(([day, dayEvents]) => <details key={day} className="eventDayBlock collapsibleDay" open={true}>
      ...
      <div className="eventCardGrid">
        {dayEvents.map(event => {
          ...
          return <div className={event.featured ? "card neonCard eventCard premiumEventCard featuredEvent" : "card neonCard eventCard premiumEventCard"} key={event.occurrenceKey || event.id}>
...
function HostEventTypeManager() {
  ...
  function deleteType(label) {
    if (!confirm(`Delete event type "${label}"?`)) return;
...
function HostEventsPanel() {
  ...
  async function saveEvent() {
    ...
    const res = await hostFetch(url, {
      ...
      body: JSON.stringify({
        ...form,
        dayOfWeek: (form.daysOfWeek && form.daysOfWeek.length ? form.daysOfWeek[0] : form.dayOfWeek),
        daysOfWeek: form.daysOfWeek && form.daysOfWeek.length ? form.daysOfWeek : [form.dayOfWeek],
  ...
  function toggleEventDay(day) {
    setForm(current => {
      const currentDays = Array.isArray(current.daysOfWeek) ? current.daysOfWeek : [current.dayOfWeek ?? 1];
      const exists = currentDays.includes(day);
      const nextDays = exists ? currentDays.filter(item => item !== day) : [...currentDays, day];
      const safeDays = nextDays.length ? nextDays.sort((a,b) => a-b) : [day];
    ...
    <div className="card quickEventBuilder">
      ...
      <div className="wizardStep">
        ...
        <div>
          ...
          <select value={customEventTypes.includes(form.customEventType) ? form.customEventType : ""} onChange={e => applyCustomEventType(e.target.value)}>
            <option value="">Choose custom event type</option>
            {customEventTypes.map(label => <option key={label} value={label}>{label}</option>)}
          </select>
...
function HostManager() {
  ...
  return <div className="screen hostManagerCompact">
    ...
    <HostSection id="host-section-events" title="Events & Calendar" subtitle="Event calendar, custom event types, booking requests, and event editor.">
      <HostEventCalendar />
    ...
    <HostSection id="host-section-marketing" title="Marketing" subtitle="Venue hero templates, QR tools, and promo assets.">
