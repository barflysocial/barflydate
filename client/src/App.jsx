...
function eventHasMeetMingle(event) {
  return event?.meetMingleEnabled !== false && Boolean(event?.meetMingleGameId);
}
...
const APP_VERSION = "BARFLYDATE v89";
...
function EventsCalendar() {
  ...
  const hasTodayRsvpEvent = events.some(event => event.dateLabel === todayKey && eventHasMeetMingle(event) && !eventUnavailable(event));
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    ...
    {hasTodayRsvpEvent && <EventsPageActionNav />}
  </div>;
}
