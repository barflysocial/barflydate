...
function eventShareTargetUrl(event) {
  const playLink = eventPlayLink(event);
  if (playLink) return absoluteAppUrl(playLink);
  const action = eventPrimaryAction(event);
  if (action?.type === "link" && action.link) return absoluteAppUrl(action.link);
  return eventUrl(event);
}

function eventQrRoute(event) {
  const targetUrl = eventShareTargetUrl(event);
  const title = encodeURIComponent(event?.title || "Barfly Event");
  return `/qr/link?url=${encodeURIComponent(targetUrl)}&title=${title}`;
}
...
function shareEvent(event) {
  const url = eventShareTargetUrl(event);
...
function EventShareButton({ event, label = "Share" }) {
  ...
  const hasDirectPlayLink = eventHasPlayLink(event);
  const qrLink = eventQrRoute(event);
  ...
  return <div className="shareMenuWrap">
    ...
    {open && <div className="shareMenuCard">
      ...
      <button type="button" onClick={copyLink}>{hasDirectPlayLink ? "Copy Play Link" : "Copy Event Link"}</button>
      <button type="button" onClick={openQr}>{hasDirectPlayLink ? "Show Play QR Code" : "Show QR Code"}</button>
...
function QrDisplay() {
  ...
  const url = kind === "link" && customUrl ? customUrl : (kind === "event" && eventSlug ? `${window.location.origin}/events/${eventSlug}` : makeQrUrl(kind));
  const labels = {
    ...
    link: { title: customTitle ? `Scan for ${customTitle}` : "Scan to Open Link", subtitle: "This QR opens the same destination as the event action button." }
  };
...
function EventDetail() {
  ...
  return <div className="screen publicScreen neonPublic eventDetailPage">
    ...
    <div className="card qrMiniCard">
      ...
      <img src={qrApiUrl(eventShareTargetUrl(event))} alt="Event QR" />
      <p className="qrUrl">{eventShareTargetUrl(event)}</p>
    </div>
    ...
    <div className={`igPosterCanvas ${posterSize} template-${template?.slug || "game-night"} layout-${overlayLayout}`}>
      ...
      <div className="igPosterOverlay">
        ...
        <div className="igPosterFooter">
          ...
          <img src={qrApiUrl(eventShareTargetUrl(event), 260)} alt="Event QR" />
        </div>
      </div>
    </div>
...
function HostEventCalendar() {
  ...
  return <div className="card glowCard hostCalendarPanel" id="host-event-calendar">
    ...
    {selectedEvents.map(event => <div className="rsvpRow hostCalendarRow" key={event.occurrenceKey || event.id}>
      ...
      <div className="hostPlayerActions playerViewActions">
        <Button className="secondary" onClick={() => window.location.href = event.publicPath}>Event Page</Button>
        <Button className="secondary" onClick={() => window.location.href = eventQrRoute(event)}>QR</Button>
...
function BusinessDemo() {
  ...
  const comparisonRows = [
    "Multiple game types in one night",
    "Digital, phone-friendly gameplay",
    "Meet & Mingle + social experiences",
    "Voting + Social Wall tools",
    "Barfly TV support",
    "Sponsor integration",
    "YouTube Shorts, TikTok, and Instagram promo links",
    "Public events, private parties, and activations",
    "Custom branding and venue-friendly pages",
    "One platform for events, games, and experiences",
    "Pricing guarantee: If we can’t beat their price, it’s free!"
  ];
    ...
    <div className="card neonCard simplifiedDemoSection comparisonDemoSection">
      ...
      <div className="comparisonTable" role="table" aria-label="Barfly Social comparison chart">
        ...
        {comparisonRows.map(feature => <React.Fragment key={feature}>
          ...
          <div className="comparisonCell comparisonBarfly comparisonCheckCell" role="cell" aria-label="Included in Barfly Social"><span className="comparisonCheck" aria-hidden="true">✓</span></div>
          <div className="comparisonCell comparisonTraditional comparisonBlankCell" role="cell" aria-label="Not included"></div>
        </React.Fragment>)}
      </div>
    </div>
