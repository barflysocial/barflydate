...
const APP_VERSION = "BARFLYDATE v41";
...
function SplashScreen() {
  ...
  const [destination, setDestination] = React.useState("/events");
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/forecast`).then(res => res.json()).catch(() => null),
      fetch(`${API}/settings`).then(res => res.json()).then(data => {
        const dest = data?.settings?.homepageDestination || "events";
        const routes = { events: "/events", play: "/play", book: "/book", demo: "/demo", forecast: "/forecast" };
        setDestination(routes[dest] || "/events");
      }).catch(() => null)
    ]).finally(() => {
  ...
  React.useEffect(() => {
    if (!ready) return;
    const t = setTimeout(() => { window.location.href = destination; }, SPLASH_FADE_MS);
...
function EventsCalendar() {
  ...
  const dayOptions = visibleEvents.reduce((acc, event) => {
    if (!acc.find(item => item.key === event.dateLabel)) {
      acc.push({ key: event.dateLabel, label: `${event.dayLabel}`, date: event.dateLabel });
    }
    return acc;
  }, []);

  const activeDayKey = selectedDay === "today" ? todayKey : selectedDay;
  const selectedEvents = selectedDay === "all"
    ? visibleEvents
    : visibleEvents.filter(event => event.dateLabel === activeDayKey);
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    ...
    <div className="card neonCard eventBrowseCard">
      <div className="brandMark">BROWSE BY DAY</div>
      <div className="dayPicker">
        <button className={selectedDay === "today" ? "chip on" : "chip"} onClick={() => setSelectedDay("today")}>Today</button>
        <button className={selectedDay === "all" ? "chip on" : "chip"} onClick={() => setSelectedDay("all")}>Show All</button>
        {dayOptions.map(day => <button key={day.key} className={selectedDay === day.key ? "chip on" : "chip"} onClick={() => setSelectedDay(day.key)}>
          {day.label}
    ...
    {data && visibleEvents.length > 0 && selectedEvents.length === 0 && <div className="card neonCard emptyDayCard">
      ...
      <p className="muted">Choose another day or tap Show All to view the full week.</p>
      <Button className="secondary" onClick={() => setSelectedDay("all")}>Show All Events</Button>
    </div>}

    {Object.entries(grouped).map(([day, dayEvents]) => <details key={day} className="eventDayBlock collapsibleDay" open={selectedDay !== "all" || dayEvents.some(event => event.dateLabel === todayKey)}>
      <summary>
        <span>{day}</span>
...
function PosterGenerator() {
  ...
  if (data.error) return <div className="screen publicScreen neonPublic"><div className="alert">{data.error}</div></div>;
...
function HostAnalyticsDashboard() {
  ...
  return <div className="card glowCard analyticsDashboard">
...
function HostBookingsPanel() {
  ...
  return <div className="card glowCard hostBookingsPanel">
    ...
    {bookings.map(booking => <div className="rsvpRow bookingRequestRow" key={booking.id}>
      <div>
        ...
        <p className="muted">{booking.eventScope === "private" ? "Private Event" : "Public Event"} • {booking.requestedDate} at {booking.requestedTime} • {booking.duration}</p>
...
function BusinessDemo() {
  ...
  return <div className="screen publicScreen neonPublic businessDemo">
    <div className="businessHero">
      ...
      <div className="ctaRow">
        <Button onClick={() => window.location.href = settings.bookingLink || BUSINESS_BOOKING_LINK}>Book a Demo</Button>
        <Button className="secondary" onClick={() => window.location.href = "/book"}>Request Event Booking</Button>
...
function HostSettingsPanel() {
  ...
  return <div className="card glowCard hostSettingsPanel">
    ...
    <div className="twoCol">
      ...
      <div><label>Homepage After Splash</label><select value={settings.homepageDestination || "events"} onChange={e => update("homepageDestination", e.target.value)}>
        <option value="events">Events</option>
        <option value="play">Play Online</option>
        <option value="book">Booking</option>
        <option value="demo">Business Demo</option>
        <option value="forecast">Barfly Social Forecast</option>
      </select></div>
    </div>
