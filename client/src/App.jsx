...
function PublicBottomActionStack({ showRsvp = false }) {
  if (!showRsvp) return null;
  return <div className="publicBottomActionStack rsvpOnlyBottomStack">
    <EventsPageActionNav />
  </div>;
}

function GlobalFixedGameBar() {
  const settings = usePublicSettings();
  const hasTools = globalIframeTools(settings).length > 0;
  if (!hasTools) return null;
  return <div className="globalFixedGameBar">
    <GlobalGameIconBar settings={settings} />
  </div>;
}
...
const APP_VERSION = "BARFLYDATE v99";
...
function SplashScreen() {
  ...
  React.useEffect(() => {
    if (!ready) return;
    const t = setTimeout(() => { window.location.href = "/events"; }, SPLASH_FADE_MS);
    ...
    <div className="splashCenter">
      ...
      {showHomeTip && <div className="homeTipCard minimalHomeTip">
        ...
        <div className="ctaRow">
          <Button onClick={continueToEvents}>OK</Button>
        </div>
      </div>}
    </div>
  </div>;
...
function EventsCalendar() {
  ...
  const cityOptions = Array.from(new Set(["Baton Rouge", ...String(settings.eventCities || "Baton Rouge").split(/[\n,]/).map(city => city.trim()).filter(Boolean)]));
  function eventMatchesCity(event, city) {
    if (!city || city === "all") return true;
    const text = [event.venueLocation, event.venueName, event.city].filter(Boolean).join(" ").toLowerCase();
    return text.includes(String(city).toLowerCase());
  }
  const cityEvents = cityFilter === "all" ? events : events.filter(event => eventMatchesCity(event, cityFilter));
  const visibleEvents = filter === "all" ? cityEvents : cityEvents.filter(event => event.eventType === filter);
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    ...
    <PublicBottomActionStack showRsvp={hasSelectedDateRsvpEvent} />
  </div>;
}
...
function SocialWall() {
  const frames = [
    ...
    {
      title: "Barfly Games",
      url: "https://games.barfly.social/",
      note: "Games home"
    }
  ];

  return <div className="screen publicScreen neonPublic socialWallPage">
    <PublicBrandHeader />
    <div className="publicHero socialWallHero">
      <div className="brandMark">SOCIAL WALL</div>
      <h1>Barfly Social Wall</h1>
      <p className="tagline">Photos, games, and live social content in one place.</p>
      <div className="ctaRow">
        <Button className="secondary" onClick={() => window.location.href = "/events"}>Back to Events</Button>
      </div>
    </div>

    <div className="socialWallFrameGrid">
      {frames.map(frame => <div className="card neonCard socialWallFrameCard" key={frame.url}>
        <div className="brandMark">{frame.note}</div>
        <h2>{frame.title}</h2>
        <iframe src={frame.url} title={frame.title} loading="lazy" scrolling="no" allow="camera; microphone; clipboard-write; fullscreen" />
...
function HostSettingsPanel() {
  ...
  return <div className="card glowCard hostSettingsPanel" id="host-settings-panel">
    ...
    <div className="twoCol">
      ...
      <div className="fullWidth"><label>Event Cities</label><textarea value={settings.eventCities || "Baton Rouge"} onChange={e => update("eventCities", e.target.value)} placeholder="Baton Rouge\nDenham Springs\nPrairieville" /><p className="microcopy">One city per line or comma-separated. These appear in the public /events Search by City dropdown.</p></div>
    </div>
    ...
    <div className="promoSettingsBox globalGameSettingsBox">
      <div className="brandMark">GLOBAL GAME IFRAME BUTTONS</div>
      <h3>Bottom App Game Buttons</h3>
      <p className="microcopy">These are standalone app-wide buttons. They are not tied to individual events. Blank URLs hide that button.</p>
      <div className="twoCol">
        <div><label>Bingo iframe URL</label><input value={settings.globalBingoIframeUrl || ""} onChange={e => update("globalBingoIframeUrl", e.target.value)} placeholder="https://games.barfly.social/bingo" /></div>
        <div><label>Trivia iframe URL</label><input value={settings.globalTriviaIframeUrl || ""} onChange={e => update("globalTriviaIframeUrl", e.target.value)} placeholder="https://games.barfly.social/trivia" /></div>
        <div><label>Mystery iframe URL</label><input value={settings.globalMysteryIframeUrl || ""} onChange={e => update("globalMysteryIframeUrl", e.target.value)} placeholder="https://games.barfly.social/mystery" /></div>
        <div><label>Vote iframe URL</label><input value={settings.globalVoteIframeUrl || ""} onChange={e => update("globalVoteIframeUrl", e.target.value)} placeholder="https://games.barfly.social/vote" /></div>
      </div>
      <GlobalGameIconBar settings={settings} />
    </div>
...
function HostHeroTemplateManager() {
  ...
  return <div className="card glowCard heroTemplateManager" id="host-hero-templates">
    ...
    {saveMessage && <div className="successBanner">✅ {saveMessage}</div>}
    <p className="muted">Create reusable hero art by venue. Choose a saved venue or type a manual venue name.</p>

    <div className="twoCol">
      <div><label>Saved Venue</label><select value={venueHeroVenues.includes(form.venueName) ? form.venueName : ""} onChange={e => update("venueName", e.target.value)}>
        <option value="">Choose saved venue or type manually</option>
        {venueHeroVenues.map(venue => <option key={venue} value={venue}>{venue}</option>)}
      </select></div>
      <div><label>Manual Venue Name</label><input value={form.venueName} onChange={e => update("venueName", e.target.value)} placeholder="Type venue name" /></div>
...
function HostEventsPanel() {
  ...
  function editEvent(event) {
    ...
    setForm({
      ...
      instagramCaption: event.instagramCaption || "",
      shortPromoText: event.shortPromoText || "",
  ...
  async function saveEvent() {
    ...
    if (form.saveQuickSetup) {
      const setupRes = await hostFetch(`${API}/host/event-templates`, {
        method: "POST",
        headers: hostHeaders(true),
        body: JSON.stringify({
          name: form.quickSetupName || form.title || form.customEventType || "Saved Quick Setup",
          eventType: payload.eventType,
          customEventType: form.customEventType,
          title: form.title,
          description: form.description,
          prizeSpecial: form.prizeSpecial,
          buttonText: form.buttonText,
          buttonLink: form.buttonLink,
          onlineOnly: !!form.onlineOnly,
          eventStatus: form.eventStatus || "scheduled",
          redemptionRules: form.redemptionRules,
          eventImageUrl: form.eventImageUrl
        })
      });
      const setupData = await setupRes.json().catch(() => ({}));
      if (!setupRes.ok || setupData.error) return alert(setupData.error || "Could not save quick setup.");
      completed.push("quick setup");
    }
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
    ...
    <div className="card quickEventBuilder">
      ...
      <div className="quickPreviewCard">
        ...
        <div>
          <label className="checkRow"><input type="checkbox" checked={!!form.saveQuickSetup} onChange={e => update("saveQuickSetup", e.target.checked)} /><span>Save this setup as a Quick Setup</span></label>
          {form.saveQuickSetup && <input value={form.quickSetupName} onChange={e => update("quickSetupName", e.target.value)} placeholder="Quick setup name" />}
          <Button onClick={saveEvent}>{form.saveQuickSetup ? "Save Event + Setup" : quickSaveLabel()}</Button>
        </div>
      </div>
    </div>

    <details className="advancedEventDetails">
      <summary><b>Advanced Event Details</b><span>Extra schedule, hero, post, ticketing, sponsor, and Meet & Mingle controls.</span></summary>

      <div className="advancedGroup">
        <div className="brandMark">CALENDAR SCHEDULE</div>
        <div className="twoCol">
          <div><label>Specific Event Date</label><input type="date" value={form.date || ""} onChange={e => update("date", e.target.value)} /><p className="microcopy">Use this for one-time events. Leave blank for weekly events.</p></div>
          ...
          <div><label>Start Time</label><input type="time" value={form.startTime} onChange={e => update("startTime", e.target.value)} /></div>
          <div><label>End Time</label><input type="time" value={form.endTime} onChange={e => update("endTime", e.target.value)} /></div>
          <div className="fullWidth"><label className="checkRow"><input type="checkbox" checked={!!form.recurringWeekly} onChange={e => update("recurringWeekly", e.target.checked)} /><span>Recurring Weekly</span></label></div>
          <div className="fullWidth"><label>Days of Week</label><div className="dayCheckGrid">{weeklyDayOptions.map(day => <label key={day.value} className="checkRow dayCheck"><input type="checkbox" checked={(form.daysOfWeek || []).includes(day.value)} onChange={() => toggleEventDay(day.value)} /><span>{day.label}</span></label>)}</div></div>
      ...
      <div className="advancedGroup">
        <div className="brandMark">HERO GRAPHIC</div>
        <label className="checkRow"><input type="checkbox" checked={form.showHeroGraphic !== false} onChange={e => update("showHeroGraphic", e.target.checked)} /><span>Use hero graphic for this event</span></label>
        <div className="twoCol">
          <div><label>Hero Image URL</label><input value={form.heroImageUrl} disabled={form.showHeroGraphic === false} onChange={e => update("heroImageUrl", e.target.value)} placeholder="Paste hero image URL or choose a saved preview" />{heroUrlWarning(form.heroImageUrl) && <p className="warningText">{heroUrlWarning(form.heroImageUrl)}</p>}</div>
          <div><label>Hero Fit</label><select value={form.heroFit || "cover"} onChange={e => update("heroFit", e.target.value)}><option value="cover">Cover / Fill Box</option><option value="contain">Contain / Show Full Image</option></select></div>
          <div><label>Event Image URL</label><input value={form.eventImageUrl} onChange={e => update("eventImageUrl", e.target.value)} placeholder="Optional flyer or backup image URL" /></div>
        </div>
      </div>

      <div className="advancedGroup">
        <div className="brandMark">PUBLIC EVENT INFO</div>
        <div className="twoCol">
          <div><label>Event Title</label><input value={form.title} onChange={e => update("title", e.target.value)} /></div>
          <div><label>Display Event Type</label><input value={form.customEventType} onChange={e => update("customEventType", e.target.value)} placeholder="Trivia • Bingo • Karaoke" /></div>
          <div><label>Primary Button Name</label><input value={form.buttonText} onChange={e => update("buttonText", e.target.value)} placeholder="Play Bingo, RSVP, More Info" /></div>
          <div><label>Primary Button Link</label><input value={form.buttonLink} onChange={e => update("buttonLink", e.target.value)} placeholder="/events or https://..." /></div>
          <div className="fullWidth"><label>Description</label><textarea value={form.description} onChange={e => update("description", e.target.value)} /></div>
          <div className="fullWidth"><label>Prize / Special</label><input value={form.prizeSpecial} onChange={e => update("prizeSpecial", e.target.value)} /></div>
        </div>
      </div>

      <div className="advancedGroup">
        <div className="brandMark">POST INFORMATION</div>
        <p className="microcopy">Internal copy/paste information for social media posts. Post Notes are not shown to public users.</p>
        <label>Instagram Caption</label>
        <textarea value={form.instagramCaption} onChange={e => update("instagramCaption", e.target.value)} placeholder="Write the full IG caption here." />
        <div className="twoCol">
          <div><label>Short Promo Text</label><input value={form.shortPromoText} onChange={e => update("shortPromoText", e.target.value)} placeholder="Quick story/reel promo line" /></div>
          <div><label>Hashtags</label><input value={form.hashtags} onChange={e => update("hashtags", e.target.value)} placeholder="#BarflySocial #BatonRougeEvents" /></div>
        </div>
        <label>Post Notes</label>
        <textarea value={form.postNotes} onChange={e => update("postNotes", e.target.value)} placeholder="Internal reminder: tag venue, include sponsor logo, post Tuesday morning..." />
      </div>

      <div className="advancedGroup">
        <div className="brandMark">TICKETING</div>
        ...
        {form.paidEvent && <div className="twoCol">
      ...
      <div className="advancedGroup">
        <div className="brandMark">SPONSOR / PRIZE RULES</div>
        <div className="twoCol">
          ...
          <div><label>Sponsor Logo URL</label><input value={form.sponsorLogoUrl} onChange={e => update("sponsorLogoUrl", e.target.value)} placeholder="https://..." /></div>
      ...
      <div className="advancedGroup meetMingleEventBox">
        <div className="brandMark">MEET & MINGLE ADVANCED</div>
        ...
        {form.meetMingleEnabled && <>
          ...
          <div className="twoCol">
            <div><label>Drink Special Title</label><input value={form.meetMingleDrinkSpecialTitle} onChange={e => update("meetMingleDrinkSpecialTitle", e.target.value)} /></div>
            <div><label>Drink Special Details</label><input value={form.meetMingleDrinkSpecialDetails} onChange={e => update("meetMingleDrinkSpecialDetails", e.target.value)} /></div>
            <div><label>Redeem Window</label><input value={form.meetMingleDrinkSpecialWindow} onChange={e => update("meetMingleDrinkSpecialWindow", e.target.value)} /></div>
            <div><label>Restrictions</label><input value={form.meetMingleDrinkSpecialRestrictions} onChange={e => update("meetMingleDrinkSpecialRestrictions", e.target.value)} /></div>
          </div>
        </>}
      </div>
    ...
    {events.map(event => <div className="rsvpRow" key={event.occurrenceKey || event.id}>
      <div>
        <b>{event.title}</b>
        <p className="muted">{event.eventTypeLabel || eventTypeCalendarLabel(event.eventType)} • {event.dateLabel || event.date || event.dayLabel} • {formatEventTime(event.startTime)}–{formatEventTime(event.endTime)}</p>
...
function HostManager() {
  ...
  return <div className="screen hostManagerCompact">
    ...
    <HostSection id="host-section-gameplay" title="Gameplay" subtitle="Create sessions, test games, active games, and run-of-show.">
      ...
      <p className="microcopy">{mixerGoalOptions.find(option => option.value === mixerGoal)?.details}</p>
      ...
      <p className="microcopy">Round apps are selected automatically by Mixer Goal and Session Length.</p>
<div className="ctaRow"><Button onClick={createGame}>Create Meet & Mingle Session</Button><Button className="secondary" onClick={duplicateLastSession}>Duplicate Last Session</Button></div>
    ...
    {games.map(game => <div className="card" key={game.id}>
      ...
      <div className="row">
        ...
        <Button className="danger" onClick={() => deleteGame(game)}>Delete Session</Button>
      </div>
    </div>)}
    </HostSection>

    <HostSection id="host-section-events" title="Events & Calendar" subtitle="Event calendar, custom event types, and event editor.">
      <HostEventCalendar />
...
function App() {
  const { parts } = useRoute();
  const withGlobalGameBar = page => <>
    {page}
    <GlobalFixedGameBar />
  </>;

  if (!parts[0]) return <SplashScreen />;
  if (parts[0] === "play") return withGlobalGameBar(<PlayOnlinePage />);
  if (parts[0] === "calendar") {
    window.location.replace("/events");
    return null;
  }
  if (parts[0] === "events" && parts[1]) return withGlobalGameBar(<EventDetail />);
  if (parts[0] === "events") return withGlobalGameBar(<EventsCalendar />);
  if (parts[0] === "tonight" || parts[0] === "this-week") return withGlobalGameBar(<TonightPage />);
  if (parts[0] === "venue" && parts[1]) return withGlobalGameBar(<VenuePage />);
  if (parts[0] === "book" || parts[0] === "booking") return withGlobalGameBar(<BookingPage />);
  if (parts[0] === "social-wall") return withGlobalGameBar(<SocialWall />);
  if (parts[0] === "business-demo" || parts[0] === "demo") return withGlobalGameBar(<BusinessDemo />);
  if (parts[0] === "qr") return withGlobalGameBar(<QrDisplay />);
  if (parts[0] === "forecast" || parts[0] === "radar") return withGlobalGameBar(<Forecast />);
  if (parts[0] === "rsvp") return withGlobalGameBar(<RSVP />);
  if (parts[0] === "checkin") return withGlobalGameBar(<CheckIn />);
  if (parts[0] === "my-rsvp") return withGlobalGameBar(<MyRSVP />);
  if (parts[0] === "join") return withGlobalGameBar(<Join />);
  ...
  return withGlobalGameBar(<EventsCalendar />);
}
