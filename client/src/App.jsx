  ...
  async function createDemoGame() {
    try {
      ...
      if (!data.players || data.players.length === 0) {
        alert("Demo game was created, but no demo players were added. Please redeploy v14 and try again.");
        return;
      }
...
function HostGame() {
  ...
  async function refresh() {
    try {
      ...
      const reportRes = await fetch(`${API}/games/${gameId}/reports`);
  ...
  return <div className="screen">
    ...
    <div className="row">
      {game.status === "lobby" && <Button onClick={() => action("start-game")}>Start {game.modeLabel || gameModeLabel(game.gameMode)}</Button>}
    ...
    {(game.players || []).map(player => <div className={player.removedAt ? "card compact reportReviewed" : "card compact"} key={player.id}>
      <div>
        <b>{player.nickname}</b>
      ...
      <div className="hostPlayerActions">
