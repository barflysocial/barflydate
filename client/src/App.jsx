...
const APP_VERSION = "BARFLYDATE v108";
...
function GamesDashboard() {
  ...
  React.useEffect(() => {
    if (!activeKey && tools[0]?.key) setActiveKey(tools[0].key);
  }, [tools.map(tool => tool.key).join("|")]);

  const activeTool = tools.find(tool => tool.key === activeKey) || tools[0] || null;

  return <div className="screen publicScreen neonPublic gamesDashboardPage focusedGamesPage">
    {tools.length ? <>
      <div className="focusedGameButtonBar">
        {tools.map(tool => <button type="button" key={tool.key} className={activeTool?.key === tool.key ? "active" : ""} onClick={() => setActiveKey(tool.key)} aria-label={tool.label}>
          <span>{tool.icon}</span>
          <b>{tool.label}</b>
        </button>)}
      </div>

      {activeTool && <div className="inlineGameFrameShell">
        <iframe
          className="barflyMobileIframe inlineGameIframe"
          src={activeTool.url}
    ...
    </> : <div className="card neonCard emptyGamesPanel">
      <h2>No game links added yet.</h2>
      <p className="muted">Add Bingo, Trivia, Mystery, or Vote iframe URLs in /host → Games / Iframes.</p>
    </div>}
...
function BarflyTVPage() {
  ...
  return <div className="screen publicScreen neonPublic barflyTvPage">
    ...
    {tvUrl && <div className="card neonCard tvFrameCard">
      <iframe className="barflyMobileIframe" src={tvUrl} title="Barfly TV" loading="lazy" scrolling="no" allow="autoplay; fullscreen; picture-in-picture; clipboard-write" />
    </div>}
  ...
  return <div className="screen publicScreen neonPublic socialWallPage">
    ...
    <div className="socialWallFrameGrid">
      {frames.map(frame => <div className="card neonCard socialWallFrameCard" key={frame.url}>
        ...
        <h2>{frame.title}</h2>
        <iframe className="barflyMobileIframe" src={frame.url} title={frame.title} loading="lazy" scrolling="no" allow="camera; microphone; clipboard-write; fullscreen" />
