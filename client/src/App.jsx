...
function eventPrimaryAction(event) {
  ...
  return {
    label: eventUnavailable(event)
      ...
      : (event?.buttonText || (isBarflySocial ? "View Barfly Radar / RSVP" : isOnline ? "Play Now" : "More Info")),
...
const APP_VERSION = "BARFLYDATE v50";
...
const categories = ["Hobbies","Travel","Food & Drink","Entertainment","Life Goals","Values","Adventure","Lifestyle","Quirks","Karaoke","Dancing","Sports","Bingo","Trivia","Murder Mystery","Escape Rooms","Reading","Working Out","Pets"];
...
const publicNavItems = [
  ...
  { key: "play", label: "Play", icon: "▶️", href: "/play" },
  { key: "forecast", label: "Radar", icon: "📡", href: "/radar" },
...
function Join() {
  ...
  return <div className="screen premiumEntry">
    ...
    <div className="row"><Button className="secondary" onClick={() => window.location.href = "/forecast"}>View Barfly Radar</Button><Button className="secondary" onClick={() => window.location.href = "/rsvp"}>RSVP</Button><Button className="secondary" onClick={() => window.location.href = "/checkin"}>Check In</Button></div>
  ...
  return <div className={`screen splashScreen ${ready ? "fadeOut" : ""}`}>
    ...
    <div className="splashCenter">
      ...
      {stage === "tutorial" && <div className="homeTipCard tutorialCard">
        ...
        <div className="ctaRow">
          <Button onClick={continueToApp}>Continue to Events</Button>
          <Button className="secondary" onClick={neverShowAgain}>Don’t Show Again</Button>
        </div>
      </div>}
    </div>
...
function QrDisplay() {
  ...
  const labels = {
    forecast: { title: "Scan for Barfly Radar", subtitle: "Find the venues and events that match your vibe." },
    radar: { title: "Scan for Barfly Radar", subtitle: "Find the venues and events that match your vibe." },
...
function EventsCalendar() {
  ...
  const selectedEvents = showAllEvents ? visibleEvents : visibleEvents.filter(event => event.dateLabel === selectedDate);
  ...
  const selectedDateText = new Date(`${selectedDate}T00:00:00`).toLocaleDateString(undefined, {
    weekday: "long",
    month: "long",
    day: "numeric",
    year: "numeric"
  ...
  return <div className="screen publicScreen neonPublic eventsPage">
    <div className="publicHero eventsHeroCompact">
      <div className="ctaRow">
        <Button onClick={() => window.location.href = "/play"}>Play Online</Button>
        <Button className="secondary" onClick={() => window.location.href = "/radar"}>📡 Barfly Radar</Button>
        <Button className="secondary" onClick={() => window.location.href = "/qr/events"}>Show Events QR</Button>
      </div>
    </div>

    {featuredEvent && <div className="card neonCard featuredEventBanner">
      {featuredEvent.eventImageUrl && <img className="eventHeroImage" src={featuredEvent.eventImageUrl} alt={featuredEvent.title} />}
      <div>
        <div className="brandMark">FEATURED EVENT</div>
        <h2>{featuredEvent.title}</h2>
        <p className="sparkLine">{featuredEvent.venueName} • {formatEventTime(featuredEvent.startTime)}–{formatEventTime(featuredEvent.endTime)}</p>
        {featuredEvent.prizeSpecial && <p>{featuredEvent.prizeSpecial}</p>}
        <div className="ctaRow">
          <Button disabled={eventUnavailable(featuredEvent)} onClick={() => openEventPrimaryAction(featuredEvent)}>{eventPrimaryAction(featuredEvent).label}</Button>
          <Button className="secondary" onClick={() => shareEvent(featuredEvent)}>Share</Button>
          <Button className="secondary" onClick={() => window.location.href = featuredEvent.qrPath || `/qr/event/${featuredEvent.slug || featuredEvent.id}`}>QR</Button>
        </div>
      </div>
    </div>}

    <div className="card neonCard eventCalendarCard">
      <div className="brandMark">EVENT CALENDAR</div>
      ...
      <div className="eventFilterRow compactFilters">
      ...
      <div className="calendarGrid dateGrid">
        {calendarDays.map(day => {
          ...
          const selected = key === selectedDate && !showAllEvents;
      ...
      <div className="ctaRow calendarControls">
        ...
        <Button className={showAllEvents ? "" : "secondary"} onClick={() => setShowAllEvents(true)}>Show All Events</Button>
      </div>
    </div>
    ...
    {data && visibleEvents.length > 0 && selectedEvents.length === 0 && <div className="card neonCard emptyDayCard">
      <h2>No events listed for this day.</h2>
      <p className="muted">Choose another date on the calendar or tap Show All Events.</p>
      <Button className="secondary" onClick={() => setShowAllEvents(true)}>Show All Events</Button>
    </div>}

    {data && selectedEvents.length > 0 && <div className="selectedDateHeader">
      <div className="brandMark">{showAllEvents ? "SHOWING ALL EVENTS" : "SELECTED DATE"}</div>
      <h2>{showAllEvents ? "All Upcoming Events" : selectedDateText}</h2>
    </div>}
...
function EventDetail() {
  ...
  return <div className="screen publicScreen neonPublic eventDetailPage">
    ...
    <div className="card neonCard">
      {event.eventImageUrl && <img className="eventDetailImage" src={event.eventImageUrl} alt={event.title} />}
...
function PlayOnlinePage() {
  ...
  const events = (data?.events || []).filter(event => {
    ...
    return link && (link.startsWith("http") || ["bingo", "trivia", "music_bingo", "mystery"].includes(event.eventType));
  });
...
function PosterGenerator() {
  ...
  return <div className="screen publicScreen neonPublic posterPage">
    <div className="row posterActions"><Button className="secondary" onClick={() => window.location.href = event.publicPath}>Event Page</Button><Button onClick={() => window.print()}>Print / Save</Button><Button className="secondary" onClick={() => copyEventCaption(event, settings)}>Copy Caption</Button></div>
    <div className="posterCanvas">
      <div className="brandMark">BARFLY ENTERTAINMENT</div>
      {event.eventImageUrl && <img className="posterImage" src={event.eventImageUrl} alt={event.title} />}
      <h1>{event.title}</h1>
      <h2>{event.venueName}</h2>
      <p className="posterTime">{event.dayLabel} • {formatEventTime(event.startTime)}–{formatEventTime(event.endTime)}</p>
      {event.prizeSpecial && <p className="posterSpecial">{event.prizeSpecial}</p>}
      {(event.prizeName || event.prizeValue) && <p className="posterSpecial">{[event.prizeName, event.prizeValue].filter(Boolean).join(" • ")}</p>}
      {event.sponsorName && <p className="posterSponsor">Sponsored by {event.sponsorName}</p>}
      <img className="posterQr" src={qrApiUrl(eventUrl(event))} alt="Event QR" />
      <p className="posterUrl">{eventUrl(event)}</p>
      <p className="microcopy">{settings.defaultDisclaimer}</p>
    </div>
  </div>;
}
...
function HostQuickActions() {
  ...
  return <div className="card glowCard hostQuickActions" id="host-quick-actions">
    ...
    <div className="quickActionGrid">
      <Button onClick={() => jump("host-events")}>Add Event</Button>
      <Button className="secondary" onClick={() => jump("host-event-calendar")}>Event Calendar</Button>
      <Button className="secondary" onClick={() => jump("host-booking-calendar")}>Booking Calendar</Button>
      <Button className="secondary" onClick={() => jump("host-bookings")}>Booking List</Button>
      <Button className="secondary" onClick={() => jump("host-analytics")}>View Analytics</Button>
      <Button className="secondary" onClick={() => jump("create-social-session")}>Create Social Session</Button>
      <Button className="secondary" onClick={() => window.location.href = "/qr/events"}>Generate QR</Button>
...
function HostTemplateManager() {
  ...
  return <div className="card glowCard hostTemplateManager" id="host-templates">
    ...
    <div className="twoCol">
      ...
      <div><label>Event Image URL</label><input value={form.eventImageUrl} onChange={e => update("eventImageUrl", e.target.value)} /></div>
    </div>
...
function Forecast() {
  ...
  return <div className="screen publicScreen neonPublic">
    <div className="publicHero">
      <div className="brandMark">BARFLY RADAR</div>
      <h1>Barfly Radar</h1>
      <p className="tagline">See where your kind of crowd is gathering. Find the venues and events that match your vibe.</p>
    ...
    {!data && <div className="card neonCard"><p>Loading radar...</p></div>}
...
function CheckIn() {
  ...
  return <div className="screen premiumEntry">
    ...
    <div className="row">
      ...
      <Button className="secondary" onClick={() => window.location.href = "/forecast"}>View Barfly Radar</Button>
...
function HostSettingsPanel() {
  ...
  return <div className="card glowCard hostSettingsPanel">
...
function HostEventsPanel() {
  ...
  async function loadVenueAndTemplateOptions() {
    const [venueRes, templateRes] = await Promise.all([
  ...
  return <div className="card glowCard hostEventsPanel" id="host-events">
    ...
    <div className="twoCol">
      ...
      <div><label>Venue Logo URL</label><input value={form.venueLogoUrl} onChange={e => update("venueLogoUrl", e.target.value)} placeholder="https://..." /></div>
      <div><label>Event Image URL</label><input value={form.eventImageUrl} onChange={e => update("eventImageUrl", e.target.value)} placeholder="https://..." /></div>
    ...
    <div className="primaryActionPreview">Public button preview: <b>{form.buttonText || (form.eventType === "barfly_social" ? "View Barfly Radar / RSVP" : form.eventType === "escape_room" ? "Start Escape Room" : form.eventType === "mystery" ? "Start Mystery" : form.onlineOnly ? "Play Now" : "More Info")}</b></div>
