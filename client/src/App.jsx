...
const APP_VERSION = "BARFLYDATE v104";
...
function BarflyTVPage() {
  ...
  return <div className="screen publicScreen neonPublic barflyTvPage">
    ...
    {tvUrl && <div className="card neonCard tvFrameCard">
      <iframe src={tvUrl} title="Barfly TV" loading="lazy" scrolling="no" allow="autoplay; fullscreen; picture-in-picture; clipboard-write" />
    </div>}

    <div className="card neonCard barflyTvScheduleCard">
      <div className="brandMark">SCHEDULE</div>
      {tvEvents.length > 0 ? <>
        <h2>Upcoming Barfly TV Events</h2>
...
function HostGameIframeSettingsPanel() {
  ...
  const [settings, setSettings] = React.useState(blank);
  ...
  async function saveGameSettings() {
    const merged = { ...lastSaved, ...settings };
    const res = await hostFetch(`${API}/host/settings`, {
      method: "POST",
      headers: hostHeaders(true),
    ...
    const next = { ...blank, ...(data.settings || merged) };
    setSettings(next);
    setLastSaved(next);
  ...
  return <div className="card glowCard hostGameIframeSettings" id="host-game-iframes">
    <div className="brandMark">GAMES / IFRAMES</div>
    <h2>Game Dashboard Links</h2>
    <p className="microcopy">These app-wide links power the Games dashboard and Barfly TV. Blank URLs hide game buttons.</p>
    {saved && <div className="successBanner">✅ {saved}</div>}
    <div className="twoCol">
      ...
      <div className="fullWidth"><label>Barfly TV Empty Schedule Message</label><textarea value={settings.barflyTvEmptyMessage || ""} onChange={e => update("barflyTvEmptyMessage", e.target.value)} placeholder="Barfly TV schedule coming soon..." /></div>
    </div>
    <GlobalGameIconBar settings={settings} />
    <div className="ctaRow">
      <Button onClick={saveGameSettings}>Save Games / Iframes</Button>
      <Button className="secondary" onClick={() => setSettings(lastSaved)}>Cancel</Button>
      <Button className="secondary" onClick={() => window.location.href = "/games"}>View Games</Button>
...
function HostManager() {
  ...
  function HostPanelShell({ title, children }) {
    return <div className="screen hostManagerCompact hostIconSectionScreen">
      <div className="row hostTopActions">
        <Button className="secondary" onClick={() => setSelectedHostSection("")}>← Back to Host</Button>
  ...
  if (selectedHostSection === "events") return <HostPanelShell title="Events">
    <HostEventCalendar />
    <HostEventTypeManager />
    <HostEventsPanel />
  </HostPanelShell>;

  if (selectedHostSection === "venues") return <HostPanelShell title="Venues">
    <HostVenueManager />
  </HostPanelShell>;

  if (selectedHostSection === "heroes") return <HostPanelShell title="Hero Graphics">
  ...
  if (selectedHostSection === "settings") return <HostPanelShell title="App Settings">
    <HostSettingsPanel />
  </HostPanelShell>;

  if (selectedHostSection === "games") return <HostPanelShell title="Games / Iframes">
  ...
  if (selectedHostSection === "reports") return <HostPanelShell title="Reports">
    <HostBookingCalendar />
