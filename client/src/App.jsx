...
const APP_VERSION = "BARFLYDATE v123";
...
function HomePage() {
  ...
  const bingoHref = normalizeExternalUrl(settings?.globalBingoIframeUrl || "");
  const triviaHref = normalizeExternalUrl(settings?.globalTriviaIframeUrl || "");
  const mysteryHref = normalizeExternalUrl(settings?.globalMysteryIframeUrl || "");
  const escapeRoomHref = normalizeExternalUrl(settings?.escapeRoomUrl || "");
  const golfSimulatorHref = normalizeExternalUrl(settings?.golfSimulatorUrl || "");
  const meetMingleHref = normalizeExternalUrl(settings?.meetMingleUrl || "");
  const voteHref = normalizeExternalUrl(settings?.globalVoteIframeUrl || "");
  ...
  const primaryTiles = [
    ...
    { label: "Jobs", icon: "💼", href: jobsHref || "/jobs", text: jobsHref ? "Open Jobs." : "Check back soon." },
  ...
  const gameCards = [
    {
      label: "Bingo",
      icon: "🎱",
      description: "Blackout Bingo • Music Bingo • Mexican Lotería Bingo",
      playHref: bingoHref ? "/game/bingo" : "",
      eventsHref: "/events?category=bingo"
    },
    {
      label: "Trivia",
      icon: "❓",
      description: "General Trivia • Themed Trivia • Music Trivia",
      playHref: triviaHref ? "/game/trivia" : "",
      eventsHref: "/events?category=trivia"
    },
    {
      label: "Mystery",
      icon: "🕵️",
      description: "Digital murder mystery and clue-based cases",
      playHref: mysteryHref ? "/game/mystery" : "/mystery",
      eventsHref: "/events?category=mystery"
    },
    {
      label: "Escape Room",
      icon: "🔐",
      description: "Digital escape rooms and timed puzzle challenges",
      playHref: escapeRoomHref ? "/game/escape-room" : "/escape-room",
      eventsHref: "/events?category=escape-room"
    },
    {
      label: "Golf Simulator",
      icon: "⛳",
      description: "Putting simulators • Coming soon: full swing simulators",
      playHref: golfSimulatorHref ? "/game/golf-simulator" : "/golf-simulator",
      eventsHref: "/events?category=golf"
    },
    {
      label: "Karaoke",
      icon: "🎤",
      description: "Karaoke • Battle Karaoke",
      playHref: karaokeHref || "/karaoke",
      eventsHref: "/events?category=karaoke"
    },
    {
      label: "Meet & Mingle",
      icon: "🤝",
      description: "Guided social mixer with RSVP, check-in, timed rounds, prompts, and connection matching",
      playHref: meetMingleHref ? "/game/meet-mingle" : "/meet-mingle",
      eventsHref: "/events?category=meet-mingle"
    },
    {
      label: "Vote",
      icon: "🗳️",
      description: "Live audience voting and event polls",
      playHref: voteHref ? "/game/vote" : "",
      eventsHref: "/events?category=vote"
    }
  ];
  ...
  function openGameAction(action) {
  ...
  return <div className="screen publicScreen neonPublic homeLauncherPage">
    ...
    <div className="appIconGrid">
      {primaryTiles.map(tile => <button type="button" key={tile.label} className="appIconTile homeActionTile" onClick={() => openTile(tile)}>
    ...
    <div className="homeGameSection">
      <div className="brandMark">PLAY</div>
      <h2>Game Experiences</h2>
      <div className="gameCategoryGrid">
        {gameCards.map(card => <div className="gameCategoryCard" key={card.label}>
          <div className="gameCategoryHeader">
            <span className="appIconEmoji">{card.icon}</span>
            <div>
              <h3>{card.label}</h3>
              <p>{card.description}</p>
            </div>
          </div>
          <div className="gameCategoryActions">
            {card.playHref ? <Button onClick={() => openGameAction({ label: card.label, href: card.playHref })}>{card.label === "Karaoke" ? "Open" : "Play"}</Button> : <Button className="secondary" disabled>No Link Yet</Button>}
            <Button className="secondary" onClick={() => openGameAction({ label: `${card.label} Events`, href: card.eventsHref })}>Events</Button>
          </div>
        </div>)}
      </div>
    </div>
