...
const APP_VERSION = "BARFLYDATE v22";
...
function Forecast() {
  ...
  return <div className="screen publicScreen neonPublic">
    ...
    {(data?.sessions || []).map(session => <div className="card neonCard" key={session.id}>
      ...
      <div className="ctaRow">{!session.locked && <Button onClick={() => window.location.href = `/rsvp?gameId=${session.id}`}>RSVP for This Session</Button>}</div>
    </div>)}
...
function RSVP() {
  ...
  if (savedRsvp) return <div className="screen publicScreen neonPublic">
    ...
    <div className="card neonCard">
      ...
      <p><b>Looking For:</b> {savedRsvp.lookingForLabel || lookingForLabel(savedRsvp.lookingFor)}</p>
  ...
  return <div className="screen publicScreen neonPublic">
    ...
    <div className="card neonCard">
      ...
      <div className="ctaRow">
        ...
        {step === 5 && <Button className={!acceptedAgreement ? "disabledBtn" : ""} onClick={submitRsvp}>Submit RSVP</Button>}
      </div>
    </div>
...
function MyRSVP() {
  ...
  const openSessions = (forecast?.sessions || []).filter(session => !session.locked);
  ...
  return <div className="screen publicScreen neonPublic">
    ...
    {rsvp && <div className="card neonCard">
      ...
      <p><b>Looking For:</b> {rsvp.lookingForLabel || lookingForLabel(rsvp.lookingFor)}</p>
      ...
      {canModify && <>
        ...
        <select value={selectedGameId} onChange={e => setSelectedGameId(e.target.value)}>
          {openSessions.map(session => <option key={session.id} value={session.id}>{session.venueName} — {session.modeLabel}</option>)}
        </select>
      </>}
...
function HostManager() {
  ...
  async function createGame() {
    const res = await fetch(`${API}/games`, {
      ...
      body: JSON.stringify({ venueName, venueLocation, drinkSpecialTitle, drinkSpecialDetails, drinkSpecialWindow, drinkSpecialRestrictions, gameMode })
    });
    const data = await res.json();
